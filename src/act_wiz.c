...
void do_howl( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->in_room == NULL || vch->in_room == NULL) continue;
	if ( vch == ch )
	    act("You howl '#C$T#n'.", ch, NULL, argument, TO_CHAR);
	...
	{
	    act4( "#r$n#n howls '#C$t#n'.", ch, argument, vch, TO_VICT );
	}
    }
...
void do_fatality( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->connected == CON_PLAYING &&
		!IS_SET(d->character->deaf, CHANNEL_INFO) )
	{
	    send_to_char( "#rFatality #p->#n ",d->character );
...
void do_pchat( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->connected == CON_PLAYING && d->character != NULL )
	{
		...
		act("$n chats '#g$t#n'.",ch,arg2,victim,TO_VICT);
	    else
		act("$n chats '#g$t#n'.",ch,arg3,d->character,TO_VICT);
	}
    }
    return;
}
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
	...
	sprintf( buf, "#rReboot by #w%s#r.#n", ch->name );
	do_echo( ch, buf );
	do_forceauto(ch,"stand");
...
void do_shutdown( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    extern bool merc_down;
    sprintf( buf, "#rShutdown by #w%s#r.#n", ch->name );
    append_file( ch, SHUTDOWN_FILE, buf );
    strcat( buf, "\n\r" );
    do_echo( ch, buf );
    do_forceauto(ch,"stand");
    do_forceauto(ch,"call all");
    do_forceauto(ch,"recall");
    do_forceauto(ch,"save");
    do_autosave(ch,"");
    merc_down = TRUE;
    return;
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  gold hp mana move primal align damcap\n\r",ch );
	...
	victim->damcap[0] = value;
	send_to_char("Ok.\n\r", ch);
	send_to_char("Your damcap has been adjusted.\n\r", victim);
	return;
    }
    ...
    if ( !str_cmp( arg2, "sex" ) )
    {
	     if ( !str_cmp( arg3, "none"   ) ) value = 0;
	else if ( !str_cmp( arg3, "male"   ) ) value = 1;
	else if ( !str_cmp( arg3, "female" ) ) value = 2;
	...
	if (value >= ch->level)
	    send_to_char( "Sorry, no can do...\n\r", ch );
	else {victim->trust = value; send_to_char("Ok.\n\r",ch);}
	return;
    }
...
Hands, Arms, About, Waist, Hold, Face.\n\r",ch); return;}
	...
	if (!IS_JUDGE(ch))
	{
	    send_to_char("You are not authorised to make an item a relic.\n\r", ch);
	    return;
	}
      if (!IS_SET(obj->quest, QUEST_RELIC))
      {
        SET_BIT(obj->quest, QUEST_RELIC);
	  send_to_char("You make the item a relic!\n\r",ch);
        return;
      }
      if (IS_SET(obj->quest, QUEST_RELIC))
      {
        REMOVE_BIT(obj->quest, QUEST_RELIC);
	  send_to_char("The item is no longer a relic.\n\r",ch);
        return;
      }
	else
      {
        send_to_char("Error....in setting QUEST_RELIC.\n\r",ch);
        return;
      }
    }

    if ( !str_cmp( arg2, "artifact" ) )
    {
        if (!IS_JUDGE(ch))
	{
	    send_to_char("You are not authorised to make an item an artifact.\n\r", ch);
	    return;
	}
      if (!IS_SET(obj->quest, QUEST_ARTIFACT))
      {
        SET_BIT(obj->quest, QUEST_ARTIFACT);
	  send_to_char("You make the item a artifact!\n\r",ch);
        return;
      }
      if (IS_SET(obj->quest, QUEST_ARTIFACT))
      {
        REMOVE_BIT(obj->quest, QUEST_ARTIFACT);
	  send_to_char("The item is no longer an artifact.\n\r",ch);
        return;
      }
	else
      {
        send_to_char("Error....in setting QUEST_ARTIFACT.\n\r",ch);
        return;
      }
    }
    ...
    if ( !str_cmp( arg, "all" ) )
    {
        ...
        for ( d = descriptor_list ; d != NULL ; d = d->next )
        {
            ...
            &&   can_see( ch, d->character ) )
            {
                ...
                act("$n forces you to '$t'.", ch, argument, d->character, TO_VICT );
...
void do_clearclass( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_VAMPIRE(victim)) mortal_vamp(victim);
...
// Remove Mage Magic levels
    ...
    victim->pcdata->power[POWER_TOTAL] = 0;
    victim->pcdata->power[POWER_CURRENT] = 0;
...
// Removes other class powers and affects
    ...
    REMOVE_BIT(victim->pcdata->bodymods, MOD_UNFOLDED);
    REMOVE_BIT(victim->pcdata->disc[C_CURRENT], DEM_UNFOLDED);
    REMOVE_BIT(victim->pcdata->disc[C_CURRENT], DEM_WINGS);
    REMOVE_BIT(victim->act, PLR_WOLFMAN);
    REMOVE_BIT(victim->pcdata->disc[C_CURRENT], DEM_HOOVES);
    REMOVE_BIT(victim->more, MORE_GUARDIAN);
    REMOVE_BIT(victim->vampaff, VAM_DISGUISED);
    REMOVE_BIT(victim->vampaff, VAM_CLAWS);
    REMOVE_BIT(victim->vampaff, VAM_NIGHTSIGHT);
    REMOVE_BIT(victim->extra, EXTRA_VICISSITUDE);
    REMOVE_BIT(victim->polyaff, POLY_ZULO);
    REMOVE_BIT(victim->more, MORE_DARKNESS);
    REMOVE_BIT(victim->more, MORE_SILENCE);
    REMOVE_BIT(victim->more, MORE_REINA);
    REMOVE_BIT(victim->more, MORE_NEUTRAL);
    REMOVE_BIT(victim->polyaff, POLY_CHANGELING);
    ...
    REMOVE_BIT(victim->polyaff, POLY_FLESHCRAFT);
    REMOVE_BIT(victim->pcdata->bodymods, MOD_TUSKS);
    REMOVE_BIT(victim->pcdata->bodymods, MOD_HORNS);
    REMOVE_BIT(victim->pcdata->bodymods, MOD_EXOSKELETON);
    REMOVE_BIT(victim->polyaff, POLY_BONECRAFT);
    REMOVE_BIT(victim->more, MORE_PLASMA);
    REMOVE_BIT(victim->more, MORE_LOYAL);
    REMOVE_BIT(victim->more, MORE_MUMMIFIED);
    REMOVE_BIT(victim->polyaff, POLY_SHADOW);
    REMOVE_BIT(victim->affected_by, AFF_ETHEREAL);
    REMOVE_BIT(victim->polyaff, POLY_HAGS);
    REMOVE_BIT(victim->more, MORE_ASHES);
    REMOVE_BIT(victim->polyaff, POLY_SPIRIT);
    REMOVE_BIT(victim->vampaff, VAM_FLYING);
    REMOVE_BIT(victim->more, MORE_EVILEYE);
    REMOVE_BIT(victim->spectype,EYE_SPELL);
    REMOVE_BIT(victim->spectype,EYE_SELFACTION);
    REMOVE_BIT(victim->spectype,EYE_ACTION);
    REMOVE_BIT(victim->vampaff, VAM_SPIRITEYES);
    REMOVE_BIT(victim->more, MORE_STAKED);
    REMOVE_BIT(victim->immune, IMM_SUNLIGHT);
    REMOVE_BIT(victim->immune, IMM_VOODOO);
    REMOVE_BIT(victim->more, MORE_COURAGE);
    ...
    REMOVE_BIT(victim->more, MORE_BRISTLES);
    REMOVE_BIT(victim->vampaff, VAM_EARTHMELDED);
    REMOVE_BIT(victim->more, MORE_ANIMAL_MASTER);
    REMOVE_BIT(victim->vampaff, VAM_FANGS);
    REMOVE_BIT(victim->polyaff, POLY_WOLF);
    REMOVE_BIT(victim->affected_by, AFF_SHADOWSIGHT);
    REMOVE_BIT(victim->polyaff, POLY_MASK);
    REMOVE_BIT(victim->vampaff, VAM_SONIC);
    REMOVE_BIT(victim->polyaff, POLY_BAT);
    REMOVE_BIT(victim->polyaff, POLY_MIST);
    REMOVE_BIT(victim->vampaff, VAM_CHANGED);
    ...
    REMOVE_BIT(victim->more, MORE_PROJECTION);
    REMOVE_BIT(victim->more, MORE_OBEAH);
    REMOVE_BIT(victim->itemaffect, ITEMA_SUNSHIELD);

    clear_stats(victim);
    FILL_STRING( victim->pcdata->conding, "" );
    FILL_STRING( victim->morph, "" );
    FILL_STRING( victim->long_descr, "" );
    FILL_STRING( victim->short_descr, victim->name );
...
void do_gift( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET( obj->quest, QUEST_NOGIFT ) )
    {
...
DO_COM( do_token )
{
    ...
    {
        value = atoi( arg1 );
	if ( !IS_IMPLEMENTOR( ch ) && ( value < 1 || value > 100 ) )
        {
	    send_to_char( "Quest token should have a value between 1 and 100.\n\r", ch );
	    return;
        }
/* Morglum : Imps can now create larger q tokens for reimb's - quests etc :) */
	else if ( IS_CREATOR( ch ) && ( value < 1 || value > 10000 ) )
	{
    ...
    FILL_STRING( obj->name, "quest token" );
...
void bind_char( CHAR_DATA *ch )
{
    ...
    if (IS_MORE(ch, MORE_MUMMIFIED)) mummify_corpse(ch, obj);
    if (IS_MORE(ch, MORE_STAKED)) staked_corpse(ch, obj);
...
void do_copyover (CHAR_DATA *ch, char * argument)
{
        ...
        for (d = descriptor_list; d ; d = d_next)
        {
                ...
                {
                         if (d->character != NULL) och = d->character;
                            else continue;

                        if (och->desc->out_compress)
                        {
                            if (!compressEnd(och->desc))
                                send_to_char("Could not disable compression, you will have to reconnect.\n", och);
                        }
