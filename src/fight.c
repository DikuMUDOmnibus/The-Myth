...
int get_speed( CHAR_DATA *ch )
{
    ...
    if ( !IS_NPC(ch) && IS_VAMPIRE(ch) && ch->pcdata->rank > 1)
	count -= ( ( ch->pcdata->rank << 1 ) / 2 );

    if (!IS_NPC(ch) && IS_WEREWOLF(ch) && get_tribe(ch, TRIBE_SILENT_STRIDERS) > 0)
      {
      if (get_tribe(ch, TRIBE_SILENT_STRIDERS) > 2) count -= 3.5;
      else count -= 1.5;
	}
    if ( !IS_NPC(ch) && IS_DEMON(ch) && IS_SET(ch->pcdata->disc[C_POWERS], DEM_SPEED) )
	count -= 3;

    if ( IS_ITEMAFF(ch, ITEMA_SPEED) ) count -= 3;

    if (ch->move > 9999) count -= 1;
    if (ch->move > 19999) count -= 2;
    if (ch->move > 29999) count -= 3;
...
// morg-mod: new fight
        dam = number_range(ch->level * 0.5, ch->level * 3 / 2)+ch->level*0.25;
	if ( wield != NULL )
	    dam += dam * 0.5;
    }
    else
	...
	else if ( wield == NULL )
	{
		...
		{dam = number_range( 25, 50 );dt += TYPE_ADD_AGG;}
	...
	dam *= 1.5;
    if ( dt >= TYPE_HIT ) dam += (dam * ((ch->wpn[dt-1000]+1) * 0.01));
    if ( !IS_NPC(victim) && IS_WEREWOLF(victim) )
    {
	if (IS_SET(victim->act, PLR_WOLFMAN)) dam *= 0.5;
	if (get_tribe(victim, TRIBE_BONE_GNAWERS) > 4) dam *= 0.5;
	if (get_tribe(victim, TRIBE_SILENT_STRIDERS) > 1 ) dam *= 0.5;
	if (wield != NULL && IS_SET(wield->spectype, SITEM_SILVER))
	    {dam *= 2;dt += TYPE_ADD_AGG;}
...
{
    if (IS_STANCE(ch, STANCE_MONKEY)) return TRUE;
    else if (IS_POLYAFF(ch, POLY_BAT)) return TRUE;
    else if (IS_POLYAFF(ch, POLY_RAVEN)) return TRUE;
    return FALSE;
}

bool can_bypass( CHAR_DATA *ch, CHAR_DATA *victim )
{
    if (IS_STANCE(ch, STANCE_VIPER)) return TRUE;
    else if (IS_STANCE(ch, STANCE_MANTIS)) return TRUE;
    else if (IS_STANCE(ch, STANCE_TIGER)) return TRUE;
    else if (IS_POLYAFF(ch, POLY_SERPENT)) return TRUE;
    else if (IS_POLYAFF(ch, POLY_WOLF)) return TRUE;
    else if ( !IS_NPC(ch) && ch->stance[0] >= 11 && ch->stance[0] <= 15 )
    {
	int stance = ch->stance[0];
	if (IS_SET(ch->pcdata->qstats[stance-11], QS_SUPER_BYPASS)) return TRUE;
    }
    return FALSE;
}
...
void update_damcap( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	case CLASS_ABOMINATION:
	    ...
	    if (get_tribe(ch,TRIBE_SHADOW_LORDS) > 0) max_dam += 100;
	    if (get_tribe(ch,TRIBE_GET_OF_FENRIS) > 1) max_dam += 100;

	    if (get_breed(ch,BREED_METIS) > 2) max_dam += 50;
	    if (get_tribe(ch,TRIBE_BONE_GNAWERS) > 1) max_dam += 50;
	...
	case CLASS_BAALI:
	    ...
	    if (IS_SET(ch->act, PLR_DEMON)) max_dam += 500;
	    else if (IS_EXTRA(ch, EXTRA_PRINCE)) max_dam += 400;
	    else if (IS_EXTRA(ch, EXTRA_SIRE)) max_dam += 300;
//	    else max_dam += 200;

	    if ( !IS_NPC(ch) && IS_DEMON(ch) && IS_SET(ch->pcdata->disc[C_POWERS], DEM_MIGHT) )
		max_dam += 200;
...
//	    break;
	    if (weather_info.moonphase == MOON_NEW)
		  ...
		  max_dam -= 75;
		break;
	case CLASS_VAMPIRE: case CLASS_LICH:
	    if (ch->pcdata->wolf > 0)
		max_dam += ch->beast;
	    if ( get_disc(ch,DISC_POTENCE) > 0)
		max_dam += (get_disc(ch,DISC_POTENCE) * 100);
	    max_dam += ( ch->pcdata->rank * 100 );
          if (weather_info.time_of_day == DAY && !IS_IMMUNE(ch, IMM_SUNLIGHT))
            max_dam -= 300;
          if (weather_info.time_of_day == NIGHT && !IS_IMMUNE(ch, IMM_SUNLIGHT))
            max_dam += 300;

          if ( IS_VAMPIRE(ch) && IS_MORE( ch, MORE_OUTCAST ) )
            max_dam += 50;

	    break;
	case CLASS_DEMON: case CLASS_NEPHANDI:
	    if (IS_SET(ch->act, PLR_DEMON)) max_dam += 500;
	    else if (IS_EXTRA(ch, EXTRA_PRINCE)) max_dam += 400;
	    else if (IS_EXTRA(ch, EXTRA_SIRE)) max_dam += 300;
	    else max_dam += 200;
		...
		max_dam += (ch->pcdata->demonic * 10);
	    break;
	case CLASS_WEREWOLF: case CLASS_SKINDANCER:
	    if (get_tribe(ch,TRIBE_SHADOW_LORDS) > 0) max_dam += 100;
	    if (get_tribe(ch,TRIBE_GET_OF_FENRIS) > 1) max_dam += 100;
	    if (IS_SET(ch->act, PLR_WOLFMAN))
		max_dam += ch->pcdata->wolf;
		...
		max_dam += (get_disc(ch,DISC_POTENCE) * 50);
	   // break;
	    	if (weather_info.time_of_day == NIGHT)
			{
			...
			if (IS_AFFECTED(ch, AFF_SHADOWPLANE)) max_dam += 300;
			if (weather_info.time_of_day == DAY)
			max_dam -= 200;

          if ( IS_WEREWOLF(ch) && IS_MORE( ch, MORE_OUTCAST ) )
            max_dam += 50;
	break;
	case CLASS_MAGE:
	    {
		max_dam = 1500;
           if (ch->pcdata->runes[1] == 2047)
            max_dam += 100;
           if (ch->pcdata->runes[2] == 1023)
            max_dam += 100;
           if (ch->pcdata->runes[3] == 15)
            max_dam += 100;
	    }
	    break;
	}
	...
	{
	    int stance = ch->stance[0];
	    if      (ch->stance[0] == STANCE_BULL)   max_dam += ch->stance[stance];
	    else if (ch->stance[0] == STANCE_DRAGON) max_dam += ch->stance[stance];
	    else if (ch->stance[0] == STANCE_TIGER)  max_dam += ch->stance[stance];
	    else if ( !IS_NPC(ch) && ch->stance[0] >= 11 && ch->stance[0] <= 15 )
	    {
	...
	case CLASS_BAALI:
		if (IS_ITEMAFF(ch, ITEMA_SUNSHIELD)) max_dam += 500;
		if (victim->in_room != NULL && victim->in_room->vnum == 30000) max_dam *= 0.6;
// Giving Demon toughness an effect.
		break;
	case CLASS_BSD:
		if (IS_WEREWOLF(ch) && get_auspice(ch, AUSPICE_AHROUN) > 3 && IS_VAMPAFF(ch, VAM_CLAWS))
		max_dam += 200;
...
// Giving Demon toughness an effect.
		if (IS_SET(victim->pcdata->disc[C_POWERS], DEM_TOUGH))
		  max_dam *= 0.9;
...
// Giving Demon toughness an effect.
	...
	case CLASS_WEREWOLF: case CLASS_SKINDANCER:
	    ...
	    if (get_tribe(victim, TRIBE_BONE_GNAWERS) > 4) max_dam -= 250;
	    break;
	}
	if (ch->stance[0] != STANCE_MONKEY)
	{
	    int stance = victim->stance[0];
	    if      (victim->stance[0] == STANCE_CRAB)    max_dam -= victim->stance[stance];
	    else if (victim->stance[0] == STANCE_DRAGON)  max_dam -= victim->stance[stance];
	    else if (victim->stance[0] == STANCE_SWALLOW) max_dam -= victim->stance[stance];
...
/* For testing
    ...
    send_to_char(buf,ch);
*/
    if ( dam > max_dam || dam < 0 ) dam = max_dam;
...
void hurt_person( CHAR_DATA *ch, CHAR_DATA *victim, int dam, bool agg_dam )
{
     ...
     */
//Morglum - Silver Fangs 5, Werewolves will NOT take agravated damage at night

    if ( agg_dam && !IS_NPC( victim ) && IS_WEREWOLF( victim )
    &&	 get_tribe( victim, TRIBE_SILVER_FANGS ) > 4
    ...
    &&   victim->wait == 0 )
	do_flee( victim, "" );
    else if (IS_AFFECTED(victim, AFF_FEAR)) do_flee(victim,"");
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (!IS_NPC(ch) && !IS_NPC(victim))
    {
       if (victim->desc)
       {
          ...
          {
             send_to_char("They are currently writing a note!\n\r",ch);
             return TRUE;
          }
       }
    }
    ...
    if (victim == NULL || victim->in_room == NULL) return TRUE;
    ...
    {
	send_to_char( "You cannot fight if you have immortal powers!\n\r", ch );
	return TRUE;
    }

    if ( victim->trust > LEVEL_BUILDER )
    {
	send_to_char( "You cannot fight someone with immortal powers!\n\r", ch );
	return TRUE;
    }
    ...
    {
	send_to_char( "You cannot fight someone who is AFK!\n\r", ch );
	return TRUE;
    }
...
bool auto_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (!IS_NPC(ch) && !IS_NPC(victim))
    {
	if (IS_NEWBIE(ch)) return TRUE;
	else if (IS_NEWBIE(victim)) return TRUE;
	if (IS_MORE(ch, MORE_LOYAL) && strlen(ch->pcdata->conding) > 1 &&
	    !str_cmp(ch->pcdata->conding, victim->name))
	    return TRUE;
	else if (strlen(ch->pcdata->love) > 1 &&
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    {
	if (get_eq_char(victim, WEAR_WIELD) != NULL) num_hands++;
	if (get_eq_char(victim, WEAR_HOLD)  != NULL) num_hands++;
	if ( ( obj = get_eq_char( victim, WEAR_WIELD ) ) == NULL
	    || obj->item_type != ITEM_WEAPON || number_percent() <= 50 )
	{
	    ...
	    if (no_parry) return FALSE;
	}
    }

    if (dt < 1000 || dt > 1012) return FALSE;
    if (!IS_NPC(ch) || ch->wpn[0] > 0) chance -= (ch->wpn[dt-1000] * 0.1);
	else                chance -= (ch->level * 0.2);
    if (!IS_NPC(ch) || victim->wpn[0] > 0) chance += (victim->wpn[dt-1000] * 0.5);
	...
	{
	    if (IS_SET(ch->act, PLR_DEMON)) chance -= 30;
	    else if (IS_EXTRA(ch,EXTRA_PRINCE)) chance -= 25;
	    else if (IS_EXTRA(ch,EXTRA_SIRE)) chance -= 20;
	    else chance -= 15;
	}
	...
	{
	    if (IS_SET(victim->act, PLR_DEMON)) chance += 30;
	    else if (IS_EXTRA(victim,EXTRA_PRINCE)) chance += 25;
	    else if (IS_EXTRA(victim,EXTRA_SIRE)) chance += 20;
	    else chance += 15;
	}
	if (!IS_NPC(victim) && IS_WEREWOLF(victim) && get_tribe(victim, TRIBE_SILENT_STRIDERS) > 0 )
	{
	    if (get_tribe(victim, TRIBE_SILENT_STRIDERS) > 2 ) chance += 10;
	    else chance += 5;
	}
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if (!IS_NPC(victim) || victim->wpn[0] > 0) chance += (victim->wpn[0] * 0.5);
	else             chance += victim->level;

    if (IS_POLYAFF(victim, POLY_SERPENT)) chance += 50;
    else if (IS_POLYAFF(victim, POLY_BAT)) chance += 100;
    else if (IS_POLYAFF(victim, POLY_RAVEN)) chance += 100;
...
void update_pos( CHAR_DATA *victim )
{
    ...
    if (!IS_NPC(victim) && victim->hit <=-11 && is_undying(victim))
    {
        victim->hit = -10;
...
void make_corpse( CHAR_DATA *ch, CHAR_DATA *maker )
{
    ...
    if (IS_NPC(maker)) corpse->questmaker = str_dup(maker->short_descr);
    else corpse->questmaker = str_dup(maker->name);

    free_string(corpse->questowner);
    if (IS_NPC(ch)) corpse->questowner = str_dup(ch->short_descr);
    else corpse->questowner = str_dup(ch->name);

    if (ch->in_room != NULL) corpse->specpower = ch->in_room->vnum;
...
void make_part( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_NPC(ch))   obj->timer = number_range(30,60);
	else              obj->timer = -1;
	if (!str_cmp(arg,"head") && IS_NPC(ch))
	    obj->value[1] = ch->pIndexData->vnum;
	else if (!str_cmp(arg,"head") && !IS_NPC(ch))
	    {ch->pcdata->chobj = obj; obj->chobj = ch;obj->timer = number_range(30,60);}
	...
	/* For blood :) KaVir */
	if (vnum == OBJ_VNUM_SPILT_BLOOD) obj->timer = 30;
	...
	if (IS_DEMON(ch)) SET_BIT( obj->quest, QUEST_FOLLOWING );
...
{
    CHAR_DATA *mount;
    if (!IS_NPC(victim) && victim->form > 0) death_message(ch,victim);
    stop_fighting( victim, TRUE );
    victim->agg = 0;
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (IS_EXTRA(victim, EXTRA_DYING)) return;
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
    ...
    xp     = UMAX( 0, xp );
    xp     = (xp * (victim->level+1)) / 3/*(gch->level+1)*/;
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    else if ( dam <=  50 ) { vs = " softly";	vp = " softly";		}
    ...
    else if ( dam <= 750 ) { vs = " incredibly hard";vp = " incredibly hard";}
    else if ( dam <= 1000) { vs = " stupendosouly hard";vp = " stupendosouly hard";}
    else if ( dam <= 1250) { vs = " unfeasably hard";vp = " unfeasably hard";}
    else if ( dam <= 2000) { vs = " impossibly hard";vp = " impossibly hard";}
    else                   { vs = " supernaturally hard";vp = " supernaturally hard";}
	...
	(IS_NPC(victim) && IS_SET(victim->act, ACT_NOPARTS)) )
    {
		...
		if (IS_VAMPAFF(ch, VAM_CLAWS) && IS_MORE(ch, MORE_HAND_FLAME))
		{
		    ...
		    sprintf( buf2, "Your flaming claw%s $N%c #w[#r%d#w]#n", vs, punct, dam );
		    sprintf( buf3, "$n's flaming claw%s you%c #w[#r%d#w]#n", vp, punct, dam );
		    critical = TRUE;
		}
		else if (IS_VAMPAFF(ch, VAM_CLAWS))
		{
		    sprintf( buf1, "$n's claw%s $N%c",  vp, punct );
		    sprintf( buf2, "Your claw%s $N%c #w[#r%d#w]#n", vs, punct, dam );
		    sprintf( buf3, "$n's claw%s you%c #w[#r%d#w]#n", vp, punct, dam );
		    critical = TRUE;
		}
		else if (IS_MORE(ch, MORE_HAND_FLAME))
		{
		    sprintf( buf1, "$n's flaming hand%s $N%c",  vp, punct );
		    sprintf( buf2, "Your flaming hand%s $N%c #w[#r%d#w]#n", vs, punct, dam );
		    sprintf( buf3, "$n's flaming hand%s you%c #w[#r%d#w]#n", vp, punct, dam );
		    critical = TRUE;
		}
		else
		{
		    sprintf( buf1, "$n%s $N%c",  vp, punct );
		    sprintf( buf2, "You%s $N%c #w[#r%d#w]#n", vs, punct, dam );
		    sprintf( buf3, "$n%s you%c #w[#r%d#w]#n", vp, punct, dam );
		}
	    }
	    else if (IS_VAMPAFF(ch, VAM_CLAWS) && IS_MORE(ch, MORE_HAND_FLAME))
	    {
	    	sprintf( buf1, "$n's flaming claw hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "Your flaming claw hits $N%s%c #w[#r%d#w]#n", vs, punct, dam );
	    	sprintf( buf3, "$n's flaming claw hits you%s%c #w[#r%d#w]#n", vp, punct, dam );
		critical = TRUE;
	    }
	    else if (IS_VAMPAFF(ch, VAM_CLAWS))
	    {
	    	sprintf( buf1, "$n's claw hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "Your claw hits $N%s%c #w[#r%d#w]#n", vs, punct, dam );
	    	sprintf( buf3, "$n's claw hits you%s%c #w[#r%d#w]#n", vp, punct, dam );
		critical = TRUE;
	    }
	    else if (IS_MORE(ch, MORE_HAND_FLAME))
	    {
	    	sprintf( buf1, "$n's flaming hand hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "Your flaming hand hits $N%s%c #w[#r%d#w]#n", vs, punct, dam );
	    	sprintf( buf3, "$n's flaming hand hits you%s%c #w[#r%d#w]#n", vp, punct, dam );
		critical = TRUE;
	    }
	    else
	    {
	    	sprintf( buf1, "$n hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "You hit $N%s%c #w[#r%d#w]#n", vs, punct, dam );
	    	sprintf( buf3, "$n hits you%s%c #w[#r%d#w]#n", vp, punct, dam );
		critical = TRUE;
	    }
    	}
    	else
		...
		if ( dt >= 0 && dt < MAX_SKILL )
		{
	    	    ...
	    	    sprintf( buf2, "Your %s strikes $N%s%c #w[#r%d#w]#n", attack, vp, punct, dam );
	    	    sprintf( buf3, "$n's %s strikes you%s%c #w[#r%d#w]#n", attack2, vp, punct, dam );
		}
		else
		{
	    	    sprintf( buf1, "$n %s $N%s%c",  attack2, vp, punct );
	    	    sprintf( buf2, "You %s $N%s%c #w[#r%d#w]#n", attack, vp, punct, dam );
	    	    sprintf( buf3, "$n %s you%s%c #w[#r%d#w]#n", attack2, vp, punct, dam );
		    critical = TRUE;
		}
	    }
		...
		switch( get_disc(victim,DISC_FORTITUDE) )
		{
		    ...
		    case 1:  per_red = number_range(10,20);  break;
		    case 2:  per_red = number_range(20,40);  break;
		    case 3:  per_red = number_range(30,60);  break;
		    case 4:  per_red = number_range(40,80);  break;
		    case 5:  per_red = number_range(50,100); break;
		}
	...
	if (IS_NPC(victim) && IS_SET(victim->act, ACT_NOPARTS)) critical = FALSE;
...
void trip( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (!IS_NPC(victim))
    {
	if (IS_VAMPIRE(victim) && IS_VAMPAFF(victim,VAM_FLYING)) return;
	if (IS_DEMON(victim) && IS_DEMAFF(victim, DEM_UNFOLDED)) return;
    }
    if ( victim->wait == 0)
    {
...
void do_decapitate( CHAR_DATA *ch, char *argument )
{
    ...
    if (victim->desc == NULL )
    {
    	send_to_char("Sorry, they are linkdead atm.\n\r", ch);
    	return;
    }
    ...
    if (ch->in_room == NULL || (IS_SET(ch->in_room->room_flags, ROOM_SAFE)/* && ch->fight_timer < 2*/))
    {
    ...
    {
    	sprintf(buf,"You recive a %dqp bounty, for killing %s.\n\r",
    	victim->pcdata->bounty, victim->name);
    	send_to_char(buf, ch);
        sprintf( buf, "%s has taken the %d bounty on %s's head", ch->name, victim->pcdata->bounty,
        victim->name);
        do_info(ch, buf);
...
if ( (( get_age(victim) - 17) * 2 ) < 5 && victim->race == 0 )
{
sprintf(buf,"#r%s has #ybeen newbie#n #rcapped by #w%s#n.",victim->name,ch->name);
do_info(ch,buf);
do_paradox(ch,"self");
...
if ((victim->race < 1 && ch->race > 0))
    {
	ch->paradox[0] += ch->race * 3;
	ch->paradox[1] += ch->race * 3;
	...
	if (IS_WEREWOLF(victim))
	{
	    if (IS_SET(victim->act, PLR_WOLFMAN)) do_unwerewolf(victim,"");
...
//	ch->fight_timer += 10;
	...
	switch (victim->pcdata->runes[0])
	{
	    ...
	    case PURPLE_MAGIC: mage_col = "purple"; break;
	    case BLUE_MAGIC:   mage_col = "blue";   break;
	    case RED_MAGIC:    mage_col = "red";    break;
	    case GREEN_MAGIC:  mage_col = "green";  break;
	    case YELLOW_MAGIC: mage_col = "yellow"; break;
	}
	...
	if (IS_AFFECTED(victim, AFF_POLYMORPH)) do_unpolymorph(victim,"");
    }
...
if ((victim->race < 1 && ch->race > 0))
  {
	...
	ch->paradox[1] += ch->race * 3;
	ch->paradox[2] += PARADOX_TICK;
    	sprintf(buf,"%s has just had their head torn of by %s for no status.",victim->name,ch->name);
	do_fatality(ch,buf);
	sprintf( log_buf, "%s decapitated by %s at %d for no status.",
	    victim->name, ch->name, victim->in_room->vnum );
	log_string( log_buf );
...
//	ch->fight_timer += 10;
    ...
    victim->pdeath = victim->pdeath + 1;
    victim->pcdata->wolf = 0;
    sprintf(buf,"%s has just had their head torn off by %s.",victim->name,ch->name);
    do_fatality(ch,buf);
    sprintf( log_buf, "%s decapitated by %s at %d.",
	victim->name, ch->name, victim->in_room->vnum );
    log_string( log_buf );
...
void death_message( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    {
	sprintf(buf,"%s has just murdered %s's unborn child!",chname,victim->name);
	buf[0] = UPPER( buf[0] );
	do_fatality(ch,buf);
    }
    return;
...
void demon_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if (IS_DEMON(ch))
	{
		...
		{
		default:
		    ch->pcdata->power[POWER_CURRENT] += victim->level *1.75;
		    ch->pcdata->power[POWER_TOTAL] += victim->level *1.75;
		    break;
		/* List of mob vnums which don't give dp's */
		case 29600: case 30000: case 30001: case 30006:
	        case 30007: case 30008: case 30009: case 30010:
		case 30013: case 30014: case 30015: case 30016:
		    break;
		}
	    }
