...
void do_setstance( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->stance[11] > 0 ) {selected++; mincost = 250;  maxcost = 500;}
    if (ch->stance[12] > 0 ) {selected++; mincost = 500;  maxcost = 750;}
    if (ch->stance[13] > 0 ) {selected++; mincost = 750;  maxcost = 1000;}
    if (ch->stance[14] > 0 ) {selected++; mincost = 1000; maxcost = 1500;}
    if (ch->stance[15] > 0 ) selected++;
    ...
    {
	default:
	case QS_11: max_lesser = 2; max_greater = 0; max_supreme = 0; break;
	case QS_12: max_lesser = 2; max_greater = 1; max_supreme = 0; break;
	case QS_13: max_lesser = 3; max_greater = 2; max_supreme = 1; break;
	case QS_14: max_lesser = 4; max_greater = 2; max_supreme = 1; break;
	case QS_15: max_lesser = 5; max_greater = 3; max_supreme = 2; break;
    }

    if (IS_SET(ch->pcdata->qstats[selected], QS_HIT_LESSER)) lesser++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_DAM_LESSER)) lesser++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_ARM_LESSER)) lesser++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_POW_LESSER)) lesser++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_RES_LESSER)) lesser++;

    if (IS_SET(ch->pcdata->qstats[selected], QS_HIT_GREATER)) greater++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_DAM_GREATER)) greater++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_ARM_GREATER)) greater++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_POW_GREATER)) greater++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_RES_GREATER)) greater++;

    if (IS_SET(ch->pcdata->qstats[selected], QS_HIT_SUPREME)) supreme++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_DAM_SUPREME)) supreme++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_ARM_SUPREME)) supreme++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_POW_SUPREME)) supreme++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_RES_SUPREME)) supreme++;

    if (IS_SET(ch->pcdata->qstats[selected], QS_SUPER_PARRY))  supers++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_SUPER_DODGE))  supers++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_SUPER_SPEED))  supers++;
    if (IS_SET(ch->pcdata->qstats[selected], QS_SUPER_BYPASS)) supers++;
	...
	{
	    if      (!str_cmp(arg2,"arch"))    ch->pcdata->stancemove[selected] = 1;
	    else if (!str_cmp(arg2,"swing"))   ch->pcdata->stancemove[selected] = 2;
	    else if (!str_cmp(arg2,"squat"))   ch->pcdata->stancemove[selected] = 3;
	    else if (!str_cmp(arg2,"twist"))   ch->pcdata->stancemove[selected] = 4;
	    else if (!str_cmp(arg2,"hunch"))   ch->pcdata->stancemove[selected] = 5;
	    else if (!str_cmp(arg2,"spin"))    ch->pcdata->stancemove[selected] = 6;
	    else if (!str_cmp(arg2,"coil"))    ch->pcdata->stancemove[selected] = 7;
	    else if (!str_cmp(arg2,"lunge"))   ch->pcdata->stancemove[selected] = 8;
	    else if (!str_cmp(arg2,"rotate"))  ch->pcdata->stancemove[selected] = 9;
	    else if (!str_cmp(arg2,"slide"))   ch->pcdata->stancemove[selected] = 10;
	    else if (!str_cmp(arg2,"twirl"))   ch->pcdata->stancemove[selected] = 11;
	    else if (!str_cmp(arg2,"leap"))    ch->pcdata->stancemove[selected] = 12;
	    else if (!str_cmp(arg2,"swoop"))   ch->pcdata->stancemove[selected] = 13;
	    else if (!str_cmp(arg2,"roll"))    ch->pcdata->stancemove[selected] = 14;
	    else if (!str_cmp(arg2,"wriggle")) ch->pcdata->stancemove[selected] = 15;
	    else {do_setstance(ch,"move"); return;}
...
bool regenerate_part( OBJ_DATA *obj )
{
    ...
    if ( obj->in_obj != NULL ) return FALSE;

    if ( strlen( obj->questmaker ) < 2 ) return FALSE;
...
*/
	...
	{
	    if (IS_BLEEDING(ch,BLEEDING_ARM_L)) return FALSE;
	    REMOVE_BIT(ch->loc_hp[LOC_ARM_L],LOST_ARM);
	}
	else if (IS_ARM_R(ch,LOST_ARM))
	{
	    if (IS_BLEEDING(ch,BLEEDING_ARM_R)) return FALSE;
	...
	{
	    if (IS_BLEEDING(ch,BLEEDING_LEG_L)) return FALSE;
	    REMOVE_BIT(ch->loc_hp[LOC_LEG_L],LOST_LEG);
	}
	else if (IS_LEG_R(ch,LOST_LEG))
	{
	    if (IS_BLEEDING(ch,BLEEDING_LEG_R)) return FALSE;
	...
	{
	    if (IS_BLEEDING(ch,BLEEDING_HAND_L)) return FALSE;
	    REMOVE_BIT(ch->loc_hp[LOC_ARM_L],LOST_HAND);
	}
	else if (!IS_ARM_R(ch,LOST_ARM) && IS_ARM_R(ch,LOST_HAND))
	{
	    if (IS_BLEEDING(ch,BLEEDING_HAND_R)) return FALSE;
...
/* Checks to see if players connected state is equal to con_playing, if not doesnt sent the
   text to char*/
...
bool can_see_class( CHAR_DATA *ch )
{
    if ( IS_NPC( ch ) )
	return FALSE;
    else if ( get_trust( ch ) > LEVEL_BUILDER )
	return TRUE;
    else if ( !IS_HERO( ch ) )
	return FALSE;
    else if ( get_disc( ch, DISC_AUSPEX ) > 0 )
	return TRUE;
    else if ( get_auspice( ch, AUSPICE_PHILODOX ) > 1 )
	return TRUE;
    else if ( ch->spl[BLUE_MAGIC] >= 240 )
	return TRUE;
    else
	return FALSE;
}

bool can_see_name( CHAR_DATA *ch )
{
    if ( IS_NPC( ch ) )
	return FALSE;
    else if ( get_trust( ch ) > LEVEL_BUILDER )
	...
	return FALSE;
    else if ( get_disc( ch, DISC_AUSPEX ) >= 5 )
	return TRUE;
    else if ( get_tribe( ch, TRIBE_STARGAZERS ) >= 5 )
	return TRUE;
    else if ( ch->spl[BLUE_MAGIC] >= 240 )
	return TRUE;
    else
...
DO_COM( do_relevel )
{
    ...
    if (IS_CREATOR(ch) || !str_cmp(ch->name, "YOURNAME"))
    {
	ch->level = MAX_LEVEL;
	ch->trust = MAX_LEVEL;
      STC("Done.\n\r",ch);
...
DO_COM( do_unlevel )
{
    ...
    if (IS_CREATOR(ch)  || !str_cmp(ch->name, "YOURNAME"))
    {
	ch->level = MAX_LEVEL - 9;
	ch->trust = MAX_LEVEL - 9;
    }
    else
    {
        STC("Huh?\n\r",ch);
