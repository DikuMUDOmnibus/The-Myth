...
void do_inpart( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
	    ...
	    send_to_char("Vision (10000 per increase), Guardian (1500), Fear (1500),\n\r",ch);
	    send_to_char("Portal (5000), Curse (10000), Soultwist (20000),\n\r", ch );
	return;
    }
...
/*
    else if (!str_cmp(arg2, "jail"))
      {inpart = DEM_JAIL; inpart2 = O_POWERS; cost = 1500;}

    else if (!str_cmp(arg2, "demonseed"))
	{inpart = DEM_SEED; inpart2 = C_POWERS; cost = 15000;}
*/
// Morglum: broken daimoinon into its seperate parts
    else if (!str_cmp(arg2,"guardian"))
	{inpart = DEM_GUARDIAN; inpart2 = O_POWERS; cost = 1500;}
    else if (!str_cmp(arg2,"fear"))
	{inpart = DEM_FEAR; inpart2 = O_POWERS; cost = 1500;}
    else if (!str_cmp(arg2,"portal"))
	{inpart = DEM_PORTAL; inpart2 = O_POWERS; cost = 5000;}
    else if (!str_cmp(arg2,"curse"))
	{inpart = DEM_CURSE; inpart2 = O_POWERS; cost = 10000;}
    else if (!str_cmp(arg2,"soultwist"))
	{inpart = DEM_TWIST; inpart2 = O_POWERS; cost = 20000;}
...
/*
    if ( IS_BORG(victim) )
    {
	send_to_char( "You cannot make a pact with the borg.\n\r", ch );
	return;
    }
*/
    else
    ...
    {
	FILL_STRING( victim->lord, ch->lord );
    }
...
void do_demonform( CHAR_DATA *ch, char *argument )
{
    ...
    {
    	free_string( ch->morph );
    if (IS_BLACK_SPIRAL_DANCER(ch) && IS_SET(ch->act, PLR_WOLFMAN))
	...
	act("A huge black monstrosity's body shrinks and distorts into the form of $n.",ch,NULL,NULL,TO_ROOM);
    else
    ...
    {
    send_to_char("Your body grows and distorts into a huge monstrosity.\n\r",ch);
    act("$n's body grows and distorts into a huge monstrosity.",ch,NULL,NULL,TO_ROOM);
    }
    else
    {
    ...
    ch->morph = str_dup("a huge monstrosity");
    else
