...
/*
    if (IS_BORG(victim))
    {
	send_to_char( "You cannot turn the borg into vampires.\n\r", ch );
	return;
    }
*/
    ...
    if (ch->vampgen == 1)
    {
	FILL_STRING( victim->lord, ch->name );
    }
    else
    {
	sprintf(buf,"%s %s",ch->lord,ch->name);
	FILL_STRING( victim->lord, buf );
    }

    if (strlen(ch->clan) > 1)
    {
	FILL_STRING( victim->clan, ch->clan );
    }
...
void do_claw( CHAR_DATA *ch, char *argument )
{
    ...
    else if (ch->vampgen == 1 || ch->vampgen == 2) can_sire = TRUE;
    else can_sire = FALSE;
...
/*
    if (IS_BORG(victim))
    {
	send_to_char( "You cannot turn the borg into werewolves.\n\r", ch );
	return;
    }
*/
    if (IS_WEREWOLF(victim))
...
void do_mask( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_disc(ch,DISC_OBFUSCATE) < 2 && get_breed(ch,BREED_HOMID) < 5)
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
void do_bloodline( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_cmp(arg1,"Morglum")) strcpy(arg1,"Morglum");

    if ( strlen(ch->clan) < 2 && ch->vampgen != 1 )
    {
	strcpy(buf,"In the name of Gaia, I announce my Garou heritage.");
	do_say(ch,buf);
	sprintf(buf,"My name is %s, I am Orphan of no tribe.", ch->name);
	do_say(ch,buf);
	return;
    }
...
void do_scry( CHAR_DATA *ch, char *argument )
{
    ...
    else if (!IS_VAMPIRE(ch) && !IS_GHOUL(ch) && !IS_ITEMAFF(ch, ITEMA_VISION)
    && !IS_WEREWOLF(ch) && !IS_DEMON(ch))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
DO_COM( do_readaura )
{
    ...
    if (IS_NPC(ch)) return;
    if ( get_disc(ch,DISC_AUSPEX) < 2
    &&	 get_tribe(ch,TRIBE_UKTENA) < 3
    &&	 get_auspice(ch,AUSPICE_THEURGE) < 5
	...
	&&	!IS_ITEMAFF(ch,ITEMA_VISION) )
	{
	    act( "You are unable to read the aura of $p.",ch,obj,NULL,TO_CHAR);
	    return;
	}
    ...
    || get_auspice( ch, AUSPICE_THEURGE ) > 4 )
	ignore_shield = TRUE;
	...
	{
	    guardian_message( victim );
	    act4("$n just failed to use Read Aura on you.",ch,NULL,victim,TO_VICT);
	}
	return;
    }
    ...
    if (!IS_NPC(victim) && IS_EXTRA(victim, EXTRA_FAKE_CON) && !ignore_shield)
    {
    ...
    {
	guardian_message( victim );
	act4("$n just used Read Aura on you.",ch,NULL,victim,TO_VICT);
    }
    return;
}
...
void werewolf_regen( CHAR_DATA *ch )
{
    ...
    {
	if ( !IS_VAMPIRE( ch ) && !IS_GHOUL( ch ) )
	    ch->blood[BLOOD_CURRENT] = UMIN(ch->blood[BLOOD_CURRENT] + (ch->blood[BLOOD_CURRENT] * 0.05), ch->blood[BLOOD_POOL]);
	...
	if ( !IS_MAGE( ch ) && !IS_ITEMAFF( ch, ITEMA_REGENERATE ) )
	    ch->mana = UMIN(ch->mana + (ch->max_mana * 0.05), ch->max_mana);
...
void do_upkeep( CHAR_DATA *ch, char *argument )
{
	...
	if (get_tribe(ch, TRIBE_SILENT_STRIDERS) > 0)
	{
		...
		send_to_char("You move at a Speed Beyond Thought.\n\r",ch);
	    else
		send_to_char("You move at the Speed of Thought.\n\r",ch);
	}
...
void do_tribe( CHAR_DATA *ch, char *argument )
{
	...
	if ( gch->clan != NULL && strlen(gch->clan) > 1 )
	    ...
	    sprintf(clan,"None");
	{
	    if ( gch->vampgen <= 2 ) strcpy(chage,"Ancient");
	    else switch (gch->pcdata->rank)
	    {
		default:
...
*/
    ...
    if (!IS_MOD(ch, MOD_TAIL))
    {
	SET_BIT(ch->pcdata->bodymods, MOD_TAIL);
	act( "A tail extends from the base of your spine.",ch,NULL,NULL,TO_CHAR);
	act( "A tail extends from the base of $n's spine.",ch,NULL,NULL,TO_ROOM);
    }
/*
...
*/
    ...
    else if (IS_ABOMINATION(ch) && IS_POLYAFF(ch, POLY_ZULO))
      sprintf(buf,"An enormous black hairy monster");
    else if ( IS_POLYAFF(ch, POLY_ZULO))
	sprintf(buf,"A gigantic furry werewolf" );
    else
	sprintf(buf,"A huge werewolf" );
    FILL_STRING( ch->morph, buf );
    if (!IS_ABOMINATION(ch))
    {
...
DO_COM( do_unwerewolf )
{
    ...
    if (IS_DEMON(ch) && IS_POLYAFF(ch, POLY_ZULO))
    {
	FILL_STRING( ch->morph, "a great demonic beast" );
    }
    else if (IS_ABOMINATION(ch) && IS_POLYAFF(ch, POLY_ZULO))
    {
	FILL_STRING( ch->morph, "a huge black monster" );
    }
    else if ( IS_POLYAFF( ch, POLY_ZULO ) )
    {
	FILL_STRING( ch->morph, "a giant" );
    }
    else
    {
	FILL_STRING( ch->morph, "" );
	...
	act("You tail retracts back into your body.",ch,NULL,NULL,TO_CHAR);
	act("$n's tail retracts back into $s body.",ch,NULL,NULL,TO_ROOM);
    }
...
DO_COM( do_favour )
{
	...
	{
	    if ( IS_WEREWOLF( ch ) )
		send_to_char( "You can't induct a Ronin!\n\r", ch );
	    else
		send_to_char( "You can't induct an Anarch!\n\r", ch );
	    return;
	}
...
void do_pray( CHAR_DATA *ch, char *argument )
{
	...
	ch->class == CLASS_NONE)
    {
	send_to_char("The Gods have restored you.\n\r",ch);
	ch->agg  = 0;
	ch->hit  = ch->max_hit;
	ch->mana = ch->max_mana;
	ch->move = ch->max_move;
...
if (!IS_JUDGE(ch) && !IS_DEMON(ch)) return;

    if (argument[0] == '\0' && (IS_JUDGE(ch) || IS_SET(ch->act, PLR_DEMON)))
    {
	...
	if ( d->connected != CON_PLAYING ) continue;
	if ( (victim = d->character) == NULL ) continue;
	if ( ( IS_NPC(victim) || !IS_DEMON(victim) ) && !IS_JUDGE( victim ) ) continue;
	if ( IS_SET(victim->deaf, CHANNEL_PRAY) ) continue;
	if ( victim == ch )
	    act("You pray '#P$T#n'.", ch, NULL, argument, TO_CHAR);
	else
	    act4( "#r$n#n prays '#P$t#n'.", ch, argument, victim, TO_VICT );
    }
    return;
}
...
bool birth_ok( CHAR_DATA *ch, char *argument )
{
    ...
    if (dad[0] == '\0')
    {
	send_to_char("You are unable to give birth - please inform Morglum.\n\r",ch);
	return FALSE;
    }
    if (child[0] == '\0')
    {
    ...
    dad[0] = UPPER(dad[0]);
    if (!str_cmp(dad,"Morglum")) strcpy(dad,"Morglum");
...
void do_powers( CHAR_DATA *ch, char *argument )
{
	...
	if (get_disc(ch,DISC_VICISSITUDE) > 3) strcat(buf," Plasmaform");
	if (get_disc(ch,DISC_VICISSITUDE) > 4) strcat(buf," Zuloform");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
    if (!any_powers)
...
void do_pigeon( CHAR_DATA *ch, char *argument )
{
    ...
    &&	get_tribe( ch, TRIBE_BONE_GNAWERS ) < 1 )
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
/*
    ...
    else if (get_tribe( ch, TRIBE_BONE_GNAWERS ) < 1 )
    {
	send_to_char("You require level 1 Bone Gnawers to call a Carrier Pigeon.\n\r",ch);
	return;
    }
*/
    if ( arg1[0] == '\0' )
...
void do_guardian( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_DEMON( ch ) && get_disc(ch,DISC_DAIMOINON) < 1 )
    {
	if ( !IS_SET( ch->pcdata->disc[O_POWERS], DEM_GUARDIAN ) )
	{
	    send_to_char( "You haven't been granted the power of guardian.\n\r", ch );
	    return;
	}
    }

    else if (get_disc(ch,DISC_DAIMOINON) < 1)
    {
...
void do_fear( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_DEMON( ch ) && get_disc(ch,DISC_DAIMOINON) < 2 )
    {
	if ( !IS_SET( ch->pcdata->disc[O_POWERS], DEM_FEAR ) )
	{
	    send_to_char( "You haven't been granted the power of fear.\n\r", ch );
	    return;
	}
    }

    else if (get_disc(ch,DISC_DAIMOINON) < 2)
    {
...
void do_cauldronblood( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( victim ) ) blood = number_range( 1, 10 );
...
void do_control( CHAR_DATA *ch, char *argument )
{
    ...
    &&	get_auspice(ch,AUSPICE_THEURGE) < 3)
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
