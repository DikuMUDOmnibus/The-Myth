...
char *	const	dir_name	[]		=
{
    "#rnorth#n", "#geast#n", "#bsouth#n", "#ywest#n", "#pup#n", "#Cdown#n"
};
...
void do_write( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
	    ...
	    {
		if ( IS_SET( obj->spectype, ADV_APPLY )
		&&  !IS_SET( obj->spectype, ADV_FAILED )
		&&   ch->class != CLASS_MAGE )
		    SET_BIT( obj->spectype, ADV_FAILED );
		...
		&& !IS_SET(obj->spectype, ADV_AGGDAM)
		&& !IS_SET(obj->spectype, ADV_FINISHED))
		SET_BIT(obj->spectype, ADV_REVERSED);

	    else if (!str_cmp(arg1,"aggravated")
		&& IS_SET(obj->spectype, ADV_STARTED)
		&& !IS_SET(obj->spectype, ADV_AGGDAM)
		&& !IS_SET(obj->spectype, ADV_REVERSED)
		&& !IS_SET(obj->spectype, ADV_FINISHED))
		{
		    SET_BIT(obj->spectype, ADV_AGGDAM);
		    obj->points += 100;
		}
	    ...
	    {
		if ( !IS_SET( obj->spectype, ADV_APPLY ) )
		    SET_BIT( obj->spectype, ADV_APPLY );
		if ( arg2[0] == '\0' )
		    SET_BIT(obj->spectype, ADV_FAILED);
		...
		if ( arg2[0] == '\0' )
		    ...
		    !str_cmp(arg2,"umbral.vision") ||
		    !str_cmp(arg2,"umbra.vision")) &&
		...
		else if (!str_cmp(arg2,"aura.no.magic") &&
		    !IS_SET(obj->value[3], AFF_AURA_NO_MAGIC))
		{
		    SET_BIT(obj->value[3], AFF_AURA_NO_MAGIC);
		    obj->points += 75;
		}
		else if (!str_cmp(arg2,"true.sight") &&
		    !IS_SET(obj->value[3], AFF_TRUE_SIGHT))
		{
		    SET_BIT(obj->value[3], AFF_TRUE_SIGHT);
		    obj->points += 75;
		}
		else if (!str_cmp(arg2,"tendrils") &&
		    !IS_SET(obj->value[3], AFF_SHADOW))
		    SET_BIT(obj->value[3], AFF_SHADOW);
		else if (!str_cmp(arg2,"rot") &&
		    !IS_SET(obj->value[3], AFF_ROT))
		{
		    SET_BIT(obj->value[3], AFF_ROT);
		    obj->points += 15;
		}
		else if (!str_cmp(arg2,"fear") &&
		    !IS_SET(obj->value[3], AFF_FEAR))
		{
		    SET_BIT(obj->value[3], AFF_FEAR);
		    obj->points += 15;
		}
		else if (!str_cmp(arg2,"jail.water") &&
		    !IS_SET(obj->value[3], AFF_JAIL))
		    SET_BIT(obj->value[3], AFF_JAIL);
		else {SET_BIT(obj->spectype, ADV_FAILED);return;}
		obj->points += 25;
	    }
...
//    CHAR_DATA *victim;
    ...
    {
	STC( "You are unable to recall to your haven.\n\r", ch );
	return;
    }
	...
	if ( IS_DEMON( ch ) )
	{
	    ...
	    &&	 number_percent( ) <= number_range( 25, 75 ) )
	    {
		STC( "You failed.\n\r", ch );
		return;
	    }
...
/* Information on what they can actually buy */
	...
	if ( ch->pcdata->perm_str < 18 )
	    { send_to_char( " Str", ch ); message = TRUE; }
	if ( ch->pcdata->perm_int < 18 )
	    { send_to_char( " Int", ch ); message = TRUE; }
	if ( ch->pcdata->perm_wis < 18 )
	    { send_to_char( " Wis", ch ); message = TRUE; }
	if ( ch->pcdata->perm_dex < 18 )
	    { send_to_char( " Dex", ch ); message = TRUE; }
	if ( ch->pcdata->perm_con < 18 )
	    { send_to_char( " Con", ch ); message = TRUE; }
	if ( !message )
	    send_to_char( "None left to train", ch );
	message = FALSE;
	send_to_char( ".\n\r", ch );

	if ( ch->level == 2 )
	    send_to_char( "Become an Avatar - 1000 exp.\n\r", ch );

	if ( ch->level > 2 && ch->race < 1 && ch->class == CLASS_NONE
	&&	!IS_IMMORTAL( ch ) )
	    send_to_char( "Become Mortal    - no cost.\n\r", ch );

	if ( ch->max_hit < CAP_HP )
	    send_to_char( "Hp               - Can train upto 1000 at a time.\n\r", ch );

	if ( ch->max_mana < CAP_MANA )
	    send_to_char( "Mana             - Can train upto 1000 at a time.\n\r", ch );

	if ( ch->max_move < CAP_MOVE )
	    send_to_char( "Move             - Can train upto 1000 at a time.\n\r", ch );

	if ( ch->practice < 100 )
	    send_to_char( "Primal           - Can train upto 50 at a time.\n\r", ch );

	send_to_char( "Immunities:", ch );
	if ( !IS_IMMUNE( ch, IMM_CHARM ) )
	    { send_to_char( " Charm", ch ); message = TRUE; }
	if ( !IS_IMMUNE( ch, IMM_SLEEP ) )
	    { send_to_char( " Sleep", ch ); message = TRUE; }
	if ( !IS_IMMUNE( ch, IMM_STEAL ) )
	    { send_to_char( " Steal", ch ); message = TRUE; }
	if ( !message )
	    send_to_char( "None left to train", ch );
	message = FALSE;
	send_to_char( ".\n\r", ch );
...
/* Let players see the cost of what they are buying */

    if ( !str_cmp( arg1, "cost" ) || !str_cmp( arg1, "costs" ) )
    {
	if ( ch->pcdata->perm_str < 18
	...
	||   ch->pcdata->perm_con < 18 )
	    send_to_char( "Stats : 200 each.\n\r", ch );

	if ( ch->max_hit < CAP_HP )
	{
	    sprintf( buf, "1 extra point of hp will cost you %d exp.\n\r",
		ch->max_hit - ( get_curr_str( ch ) + get_curr_con( ch ) ) );
	    send_to_char( buf, ch );
	}

	if ( ch->max_mana < CAP_MANA )
	{
	    sprintf( buf, "1 extra point of mana will cost you %d exp.\n\r",
		ch->max_mana - ( get_curr_wis( ch ) + get_curr_int( ch ) ) );
	    send_to_char( buf, ch );
	}

	if ( ch->max_move < CAP_MOVE )
	{
	    sprintf( buf, "1 extra point of move will cost you %d exp.\n\r",
		ch->max_move - ( get_curr_dex( ch ) + get_curr_con( ch ) ) );
	    send_to_char( buf, ch );
	}

	if ( ch->practice < 100 )
	{
	    sprintf( buf, "1 extra point of primal will cost you %d exp.\n\r",
		( 1 + ch->practice ) * 500 );
	    send_to_char( buf, ch );
	}

	sprintf( buf, "Immunities : %d exp.\n\r",
	    count_imms( ch ) );
	send_to_char( buf, ch );
	return;
    }

    if ( ch->level == 2 && !str_cmp( arg1, "avatar" ) )
    {
	if ( ch->exp < 1000 )
	{
	    send_to_char( "You require 1000 exp to become an avatar.\n\r", ch );
	    return;
	}
	ch->exp -= 1000;
	ch->level = 3;
    ...
    if ( ch->level > 2 && ch->race < 1 && ch->class == CLASS_NONE
    &&	!str_cmp( arg1, "mortal" ) && !IS_IMMORTAL( ch ) )
    {
	send_to_char( "You become mortal again!\n\r", ch );
	ch->level = 2;
	ch->trust = 0;
	return;
    }
    ...
    if ( ch->pcdata->perm_str < 18 && !str_cmp( arg1, "str" ) )
    {
	if ( ch->exp < cost )
	{
	    sprintf( buf, "That will cost you %d exp, you only have %d exp.\n\r",
		cost, ch->exp );
	    send_to_char( buf, ch );
	    return;
	}
	ch->exp -= cost;
	ch->pcdata->perm_str++;
	send_to_char( "You become stronger.\n\r", ch );
	return;
    }

    if ( ch->pcdata->perm_int < 18 && !str_cmp( arg1, "int" ) )
    {
	if ( ch->exp < cost )
	{
	    sprintf( buf, "That will cost you %d exp, you only have %d exp.\n\r",
	...
	ch->exp -= cost;
	ch->pcdata->perm_int++;
	send_to_char( "You become more intelligent.\n\r", ch );
	return;
    }

    if ( ch->pcdata->perm_wis < 18 && !str_cmp( arg1, "wis" ) )
    {
	if ( ch->exp < cost )
	{
	    sprintf( buf, "That will cost you %d exp, you only have %d exp.\n\r",
	...
	ch->exp -= cost;
	ch->pcdata->perm_wis++;
	send_to_char( "You become wiser.\n\r", ch );
	return;
    }

    if ( ch->pcdata->perm_dex < 18 && !str_cmp( arg1, "dex" ) )
    {
	if ( ch->exp < cost )
	{
	    sprintf( buf, "That will cost you %d exp, you only have %d exp.\n\r",
	...
	ch->exp -= cost;
	ch->pcdata->perm_dex++;
	send_to_char( "You become more dexterious.\n\r", ch );
	return;
    }

    if ( ch->pcdata->perm_con < 18 && !str_cmp( arg1, "con" ) )
    {
	if ( ch->exp < cost )
	{
	    sprintf( buf, "That will cost you %d exp, you only have %d exp.\n\r",
    ...
    if ( ch->practice < 100 && !str_cmp( arg1, "primal" ) )
    {
	if ( value < 0 || value > 50 )
	{
	    send_to_char( "Training value range is 1 to 50.\n\r", ch );
	    return;
	}
	if ( ( ch->practice + value ) > 100 )
	{
	    sprintf( buf, "You can only train another %d points of Primal.\n\r",
		100 - ch->practice );
	    send_to_char( buf, ch );
	    return;
	}
	...
	ch->exp -= cost;
	ch->practice += value;
	sprintf( buf, "You gain %d points of Primal.\n\r", value );
	send_to_char( buf, ch );
	return;
    }

    if ( ch->max_hit < CAP_HP && !str_cmp( arg1, "hp" ) )
    {
	if ( value < 0 || value > 1000 )
	{
	    send_to_char( "Training value range is 1 to 1000.\n\r", ch );
	    return;
	}
	if ( ( ch->max_hit + value ) > CAP_HP )
	{
	    sprintf( buf, "You can only train another %d Hit Points.\n\r",
		CAP_HP - ch->max_hit );
	    send_to_char( buf, ch );
	    return;
	}
	...
	for ( i = 0 ; i < value ; i++ )
	{
	    cost += amount - ( get_curr_str( ch ) + get_curr_con( ch ) );
	    amount++;
	}
	if ( cost < 1 ) cost = 1;
	if ( ch->exp < cost )
	...
	ch->exp -= cost;
	ch->max_hit += value;
	sprintf( buf, "You gain %d Hit Points.\n\r", value );
	send_to_char( buf, ch );
	return;
    }

    if ( ch->max_mana < CAP_MANA && !str_cmp( arg1, "mana" ) )
    {
	if ( value < 0 || value > 1000 )
	{
	    send_to_char( "Training value range is 1 to 1000.\n\r", ch );
	...
	if ( ( ch->max_mana + value ) > CAP_MANA )
	{
	    sprintf( buf, "You can only train another %d points of Mana.\n\r",
		CAP_MANA - ch->max_mana );
	    send_to_char( buf, ch );
	    return;
	}
	amount = ch->max_mana;
	...
	for ( i = 0 ; i < value ; i++ )
	{
	    cost += amount - ( get_curr_wis( ch ) + get_curr_int( ch ) );
	    amount++;
	}
	if ( cost < 1 ) cost = 1;
	if ( ch->exp < cost )
	...
	ch->exp -= cost;
	ch->max_mana += value;
	sprintf( buf, "You gain %d points of Mana.\n\r", value );
	send_to_char( buf, ch );
	return;
    }

    if ( ch->max_move < CAP_MOVE && !str_cmp( arg1, "move" ) )
    {
	if ( value < 0 || value > 1000 )
	{
	    send_to_char( "Training value range is 1 to 1000.\n\r", ch );
	...
	if ( ( ch->max_move + value ) > CAP_MOVE )
	{
	    sprintf( buf, "You can only train another %d points of Movement.\n\r",
		CAP_MOVE - ch->max_move );
	    send_to_char( buf, ch );
	    return;
	}
	amount = ch->max_move;
	...
	for ( i = 0 ; i < value ; i++ )
	{
	    cost += amount - ( get_curr_dex( ch ) + get_curr_con( ch ) );
	    amount++;
	}
	if ( cost < 1 ) cost = 1;
	if ( ch->exp < cost )
	...
	ch->exp -= cost;
	ch->max_move += value;
	sprintf( buf, "You gain %d points of Movement.\n\r", value );
	send_to_char( buf, ch );
	return;
    }

    if ( ( !str_cmp( arg1, "sleep" ) && !IS_IMMUNE( ch, IMM_SLEEP ) )
    ||	 ( !str_cmp( arg1, "charm" ) && !IS_IMMUNE( ch, IMM_CHARM ) )
    ||	 ( !str_cmp( arg1, "steal" ) && !IS_IMMUNE( ch, IMM_STEAL ) ) )
    {
	cost = count_imms( ch );
	if ( cost < 1 ) cost = 1;
	if ( ch->exp < cost )
	...
	ch->exp -= cost;
	if ( !str_cmp( arg1, "sleep" ) )
	{
	    SET_BIT( ch->immune, IMM_SLEEP );
	    send_to_char( "You are now immune to sleep spells.\n\r", ch );
	    return;
	}

	if ( !str_cmp( arg1, "charm" ) )
	{
...
void add_tracks( CHAR_DATA *ch, int direction )
{
    ...
    if (IS_NPC(ch)) return;
    if (get_auspice(ch, AUSPICE_RAGABASH) > 0) return;
