...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
	...
	buf[0] = '\0';
	if ( IS_SET(obj->quest, QUEST_ARTIFACT)) strcat(buf, "(#wArtifact#n) " );
	else if ( IS_SET(obj->quest, QUEST_RELIC)) strcat(buf, "(#rRelic#n) " );
	else if ( obj->points > 0 )
	{
		if (IS_SET(obj->quest, QUEST_IMPROVED)) strcat(buf, "(#BMythical#n) " );
		else strcat(buf, "(#yLegendary#n) " );
	}
	if ( IS_SET(obj->quest, QUEST_BLOODAGONY)) strcat(buf, "(#RBleeding#n) " );
	if ( IS_OBJ_STAT(obj, ITEM_INVIS)     )   strcat( buf, "(Invis) "     );
	if ( IS_AFFECTED(ch, AFF_DETECT_EVIL)
		    && !IS_OBJ_STAT(obj, ITEM_ANTI_GOOD)
		    && IS_OBJ_STAT(obj, ITEM_ANTI_EVIL)   )   strcat( buf, "(#bBlue Aura#n) "  );
	else if ( IS_AFFECTED(ch, AFF_DETECT_EVIL)
		    && IS_OBJ_STAT(obj, ITEM_ANTI_GOOD)
		    && !IS_OBJ_STAT(obj, ITEM_ANTI_EVIL)   )   strcat( buf, "(#rRed Aura#n) "  );
	else if ( IS_AFFECTED(ch, AFF_DETECT_EVIL)
		    && IS_OBJ_STAT(obj, ITEM_ANTI_GOOD)
		    && !IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL)
		    && IS_OBJ_STAT(obj, ITEM_ANTI_EVIL)   )   strcat( buf, "(#yYellow Aura#n) "  );
	...
	if ( IS_OBJ_STAT(obj, ITEM_SHADOWPLANE) &&
		    obj->in_room != NULL &&
		    !IS_AFFECTED(ch,AFF_SHADOWPLANE) )    strcat( buf, "(Umbra) " );
	if (!IS_OBJ_STAT(obj, ITEM_SHADOWPLANE) &&
		    obj->in_room != NULL &&
		    IS_AFFECTED(ch,AFF_SHADOWPLANE) )    strcat( buf, "(Normal plane) " );
	if ( !IS_NPC(ch) && IS_VAMPIRE(ch) &&
		    IS_OBJ_STAT(obj, ITEM_VANISH) &&
		    get_disc(ch,DISC_CHIMERSTRY) > 0 )    strcat( buf, "(Illusion) "   );
	if ( !IS_NPC(ch) && IS_WEREWOLF(ch) &&
		    IS_SET(obj->spectype, SITEM_SILVER) &&
		    get_auspice(ch,AUSPICE_AHROUN) > 1 )  strcat( buf, "(#CSilver#n) "   );

	if ( fShort )
	{
		if ( obj->short_descr != NULL )
			strcat( buf, obj->short_descr );
		if ( obj->condition < 100) strcat(buf, " (#F#rdamaged#n)");
	}
	else
	    {
...
int get_disc( CHAR_DATA *ch, int disc )
{
    ...
    if (ch->class != CLASS_VAMPIRE) max = 4;

    if ( IS_DEMON( ch ) && disc == DISC_AUSPEX ) max = 5;

    if (min_disc > max) min_disc = max;
	...
	{
	    if (ch->pcdata->powers[disc] > max) return max;
	    else return ch->pcdata->powers[disc];
	}
	else
	    return min_disc;
    }
    ...
    if (total_disc > max) total_disc = max;
...
{
	int max = 5;
	if (disc < TRIBE_BLACK_FURIES || disc > TRIBE_MAX)
	{
		bug("Get_tribe: Out of tribe range.", 0);
		return 0;
	}
	if (IS_NPC(ch)) return 0;
	if ((IS_ABOMINATION(ch) || IS_SKINDANCER(ch) || IS_BSD(ch)) &&
		    ch->pcdata->cland[2] != disc) max = 3;
...
int char_hitroll( CHAR_DATA *ch )
{
	...
	if (!IS_NPC(ch))
	{
		...
		if (get_tribe(ch,TRIBE_RED_TALONS) > 1) hr += 50;
		...
		if (IS_MAGE(ch) && ch->pcdata->runes[1] == 2047) hr += 75;
		if (IS_MAGE(ch) && ch->pcdata->runes[2] == 1023) hr += 75;
		if (IS_MAGE(ch) && ch->pcdata->runes[3] == 15) hr += 75;
		...
		if (IS_POLYAFF(ch, POLY_WOLF)) hr += 200;
		else if (IS_POLYAFF(ch, POLY_SERPENT)) hr += 150;
		else if (IS_WEREWOLF(ch) && IS_POLYAFF(ch, POLY_ZULO)) hr += 100;
		else if (IS_POLYAFF(ch, POLY_ZULO)) hr += 200;
	}

	if (!IS_NPC(ch) && ch->pcdata->wolf > 0) return hr;

	//    if (!IS_NPC(ch) && ch->pcdata->exercise[EXE_RECOVER] > 0) hr *= 0.75;
	...
	if (ch->hit < ch->max_hit && !IS_MORE(ch, MORE_NOPAIN))
	{
		if (!IS_NPC(ch) && IS_WEREWOLF(ch) && get_auspice(ch, AUSPICE_PHILODOX) > 0)
			return hr;
		// Morglum - Gives demon toughness a use
      if (!IS_NPC(ch) && IS_DEMON(ch) && IS_SET(ch->pcdata->disc[C_POWERS], DEM_TOUGH))
			return hr;
		min = hr * 0.75;
...
int char_damroll( CHAR_DATA *ch )
{
	...
	if (!IS_NPC(ch))
	{
		...
		if (IS_MAGE(ch) && ch->pcdata->runes[1] == 2047) dr += 50;
		if (IS_MAGE(ch) && ch->pcdata->runes[2] == 1023) dr += 50;
		if (IS_MAGE(ch) && ch->pcdata->runes[3] == 15) dr += 50;
		...
		if (IS_POLYAFF(ch, POLY_WOLF)) dr += 200;
		else if (IS_WEREWOLF(ch) && IS_POLYAFF(ch, POLY_ZULO)) dr += 100;
...
int char_ac( CHAR_DATA *ch )
{
	...
	if (!IS_NPC(ch))
	{
		...
		if (IS_MAGE(ch) && ch->pcdata->runes[1] == 2047) a_c -= 400;
		if (IS_MAGE(ch) && ch->pcdata->runes[2] == 1023) a_c -= 300;
		if (IS_MAGE(ch) && ch->pcdata->runes[3] == 15) a_c -= 300;

		if (IS_WEREWOLF(ch) && get_tribe(ch, TRIBE_CHILDREN_OF_GAIA) > 1) a_c -= 250;
		if (IS_POLYAFF(ch, POLY_WOLF)) a_c -= 100;
		else if (IS_POLYAFF(ch, POLY_SERPENT)) a_c -= 200;
		else if (IS_POLYAFF(ch, POLY_BAT)) a_c -= 300;
		else if (IS_POLYAFF(ch, POLY_ZULO)) a_c -= 200;
	}

	if (!IS_NPC(ch) && ch->pcdata->wolf > 0) return a_c;

	//    if (!IS_NPC(ch) && ch->pcdata->exercise[EXE_RECOVER] > 0) a_c += 250;
    else if (ch->move < ch->max_move)
	{
		if (ch->move < 1 || ch->max_move < 1)
			a_c += 250;
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	if ( IS_HEAD(victim, LOST_HEAD) && IS_AFFECTED(victim, AFF_POLYMORPH))
		...
		if ( IS_SET(victim->extra, EXTRA_AFK)         ) strcat( buf, "(#r->AFK<-#n) " );
		if ( IS_MORE(victim, MORE_SPIRIT)         ) strcat( buf, "(Spirit World) " );
		...
		if ( IS_AFFECTED(victim, AFF_FAERIE_FIRE) ) strcat( buf, "(#pPink Aura#n) "  );
		if ( IS_EVIL(victim)
			    &&   IS_AFFECTED(ch, AFF_DETECT_EVIL)     ) strcat( buf, "(#rRed Aura#n) "   );
		...
		//Werewolf illusions
			    ...
			    IS_EXTRA(victim,EXTRA_ILLUSION) &&
			    get_tribe(ch,TRIBE_STARGAZERS) > 2)	    strcat( buf, "(Illusion) " );
	...
	if ( !IS_NPC(victim) && IS_VAMPAFF(victim,VAM_EARTHMELDED))
		strcat( buf, "(EM) " );

	if ( victim->desc != NULL
		    && ( victim->desc->connected == CON_AEDITOR
		    ||	 victim->desc->connected == CON_REDITOR
		    ||	 victim->desc->connected == CON_MEDITOR
		    ||	 victim->desc->connected == CON_MEDITOR ) )
		strcat( buf, "(OLC) " );

	if ( can_see_class( ch ) /* && !IS_CHEAT( victim ) */ )
	{
		switch ( victim->class )
		{
		default:
			break;
		case CLASS_VAMPIRE:
			strcat( buf, "(#yVampire#n) " );
			break;
		case CLASS_WEREWOLF:
			strcat( buf, "(#yWerewolf#n) " );
			break;
		case CLASS_ABOMINATION:
			strcat( buf, "(#yAbomination#n) " );
			break;
		case CLASS_MAGE:
			strcat( buf, "(#yMage#n) " );
			break;
	...
	*/
	...
	if ( IS_EXTRA( victim, TIED_UP )
		    ||	 IS_EXTRA( victim, BLINDFOLDED )
		    ||	 IS_EXTRA( victim, GAGGED ) )
	{
		if ( IS_EXTRA( victim, TIED_UP )
			    &&   IS_EXTRA( victim, BLINDFOLDED )
			    &&   IS_EXTRA( victim, GAGGED ) )
		{
			if ( IS_NPC( victim ) )
				sprintf( buf3, "...%s is tied up, gagged and blindfolded!\n\r",
					victim->short_descr );
		...
		else if ( IS_EXTRA( victim, TIED_UP )
			    &&	  IS_EXTRA( victim, BLINDFOLDED ) )
		{
			if ( IS_NPC( victim ) )
				sprintf( buf3, "...%s is tied up and blindfolded!\n\r",
					victim->short_descr );
			else if ( IS_AFFECTED( victim, AFF_POLYMORPH ) )
				sprintf( buf3, "...%s is tied up and blindfolded!\n\r",
					victim->morph );
		...
		{
			if ( IS_NPC( victim ) )
				sprintf( buf3, "...%s is tied up and gagged!\n\r",
					victim->short_descr );
			else if ( IS_AFFECTED( victim, AFF_POLYMORPH ) )
				sprintf( buf3, "...%s is tied up and gagged!\n\r",
					victim->morph );
		...
		{
			if ( IS_NPC( victim ) )
				sprintf( buf3, "...%s is blindfolded and gagged!\n\r",
					victim->short_descr );
			else if ( IS_AFFECTED( victim, AFF_POLYMORPH ) )
				sprintf( buf3, "...%s is blindfolded and gagged!\n\r",
					victim->morph );
	...
	if ((mount = victim->mount) != NULL && victim->mounted == IS_RIDING)
	{
		...
		case POS_SLEEPING:
			...
			else strcat( buf, " is sleeping here." );
			break;
		case POS_RESTING:
			if (victim->sat != NULL)
			{
			...
			else strcat( buf, " is resting here." );
			break;
		case POS_MEDITATING:
			...
			else strcat( buf, " is meditating here." );
			break;
		...
		case POS_SITTING:
			...
			else strcat( buf, " is sitting here." );
			break;
...
void evil_eye( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	if (victim == ch) return;
	if (IS_IMMORTAL(ch) || IS_IMMORTAL(victim)) return;
	if (!IS_MORE(victim, MORE_EVILEYE)) return;
	if (victim->pcdata->evileye > 0) return;
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	if (IS_WEREWOLF(victim) && IS_VAMPAFF(victim, VAM_NIGHTSIGHT)) act("$N's eyes are glowing bright green.",ch,NULL,victim,TO_CHAR);
	else if (IS_AFFECTED(victim, AFF_INFRARED) || IS_VAMPAFF(victim, VAM_NIGHTSIGHT)) act("$N's eyes are glowing bright red.",ch,NULL,victim,TO_CHAR);
	...
	for ( rch = list; rch != NULL; rch = rch->next_in_room )
	{
		...
		if ( !IS_NPC(rch) )
		{
			if (IS_HEAD(rch,LOST_HEAD)
				    ...
				    || IS_VAMPAFF(rch, VAM_EARTHMELDED)) continue;
		}
...
DO_COM( do_look )
{
	...
	// Morglum : Was get_disc(ch,DISC_PROTEAN) > 7
    if ( !IS_NPC(ch) && IS_VAMPIRE(ch)
		    ...
		    &&   room_is_dark( ch->in_room ) )
	{
		if ( IS_MAGE( ch ) )
		{
			if ( ch->spl[BLUE_MAGIC] < 240 || !IS_AFFECTED( ch, AFF_INFRARED ) )
			{
	...
	if ( arg1[0] == '\0' || !str_cmp( arg1, "auto" ) )
	{
		...
		{
			/* Thanks to Satyricon for this vnum viewing bit - Morglum */
			if(IS_IMMORTAL(ch))
			{
				sprintf(buf,"%s #C[#w%i#C]#n\n\r",ch->in_room->name,ch->in_room->vnum);
			}
			else
			    {
				sprintf(buf,"%s\n\r",ch->in_room->name);
...
void do_score( CHAR_DATA *ch, char *argument )
{
	...
	sprintf( buf,
		"Str: %-7d  Int: %-7d  Wis: %-7d  Dex: %-7d  Con: %-7d\n\r",
		get_curr_str(ch),
		get_curr_int(ch),
		get_curr_wis(ch),
		get_curr_dex(ch),
	...
	sprintf( buf,
		"Autoexit: %s.\t    Items carried:  %d/%d.\n\r",
		(!IS_NPC(ch) && IS_SET(ch->act, PLR_AUTOEXIT)) ? "yes" : "no",
		ch->carry_number, can_carry_n(ch) );
	send_to_char( buf, ch );
	sprintf( buf,
		"Autoloot: %s.\t    Weight carried: %d/%d lbs.\n\r",
		(!IS_NPC(ch) && IS_SET(ch->act, PLR_AUTOLOOT)) ? "yes" : "no",
		ch->carry_weight, can_carry_w(ch) );
	send_to_char( buf, ch );
	sprintf( buf,
		"Autosac:  %s.\t    Gold carried:   %d.\n\r",
		(!IS_NPC(ch) && IS_SET(ch->act, PLR_AUTOSAC) ) ? "yes" : "no", ch->gold  );
	send_to_char( buf, ch );

	send_to_char("\n\r", ch);

	sprintf( buf,
		"Hit Points:    %5d/%-5d    Experience: %d\n\r",
		ch->hit, ch->max_hit, ch->exp);
	send_to_char( buf, ch );

	sprintf( buf,
		"Mana:          %5d/%-5d    Primal: %d\n\r",
		ch->mana, ch->max_mana, ch->practice);
	send_to_char( buf, ch );

	sprintf( buf,
		"Movement:      %5d/%-5d    Quest Points: %d    Wimpy: %d\n\r",
		ch->move, ch->max_move, ch->pcdata->quest, ch->wimpy);
	send_to_char( buf, ch );
	...
	if ( !IS_NPC(ch) && IS_EXTRA(ch, EXTRA_DONE) )
		sprintf(buf,"You are no longer a virgin.\n\n\r");
	else sprintf(buf,"\n\r");
	send_to_char( buf, ch );

	sprintf( buf, "Hitroll: %4d         ", char_hitroll(ch));
	send_to_char( buf, ch );
	if ( ch->level >= 0 )
	{
		sprintf( buf, "Armor class:%6d  ", a_c );
		send_to_char( buf, ch );
	}
	...
	sprintf( buf, "Damroll: %4d         ", char_damroll(ch));
	send_to_char( buf, ch );

	if ( ch->level >= 0 )
	{
		sprintf( buf, "Alignment:%8d  ", ch->alignment );
		send_to_char( buf, ch );
	}
	...
	if (!IS_NPC(ch))
	{
		if      (ch->level == 1 ) send_to_char( "You are a Newbie.\n\r", ch);
		else if (ch->level == 2 ) send_to_char( "You are a Mortal.\n\r", ch);
		else if (ch->level == 7 ) send_to_char( "You are a Builder.\n\r", ch);
		else if (ch->level == 8 ) send_to_char( "You are a Quest Maker.\n\r", ch);
		else if (ch->level == 9 ) send_to_char( "You are an Enforcer.\n\r", ch);
		else if (ch->level == 10) send_to_char( "You are a Judge.\n\r", ch);
		else if (ch->level == 11) send_to_char( "You are a High Judge.\n\r", ch);
		else if (ch->level == 12) send_to_char( "You are an Implementor.\n\r", ch);
		else if (ch->race >= 0 && ch->race <= 100 )
		{
			stcprintf( ch, "You are %s.\n\r", status_name( ch->race, ch->class, TRUE ) );
		}
		else send_to_char( "a Bugged Character!\n\r", ch);
	}
	...
	if (IS_MAGE(ch))
	{
		sprintf(buf, "Max number of chain pages: %d. \n\r",MAX_CHAIN(ch));
		send_to_char(buf,ch);
	}
	...
	if ( !IS_NPC(ch) )
	{
		...
		else if ( IS_DEMON(ch) && ch->pcdata->demonic > 0 )
		{
			...
			if (ch->class == CLASS_DEMON) bonus <<=1;
	...
	if (!IS_NPC(ch) && (weather_info.moonphase == MOON_HALF) && (weather_info.time_of_day == NIGHT) &&
		    IS_WEREWOLF(ch) && !IS_ABOMINATION(ch) && !IS_SKINDANCER(ch) && !IS_BLACK_SPIRAL_DANCER(ch))
	{
		send_to_char("The half moon is affecting you.\n\r",ch);
		send_to_char("#C-----------------------------------------------------------------------#n\n\r",ch);
	}
	if (!IS_NPC(ch) && (weather_info.moonphase == MOON_FULL) && (weather_info.time_of_day == NIGHT) &&
		    IS_WEREWOLF(ch) && !IS_ABOMINATION(ch) && !IS_SKINDANCER(ch) && !IS_BLACK_SPIRAL_DANCER(ch))
	{
		send_to_char("The full moon is affecting you.\n\r",ch);
		send_to_char("#C-----------------------------------------------------------------------#n\n\r",ch);
	}
	return;
}
...
void do_time( CHAR_DATA *ch, char *argument )
{
	...
	sprintf( buf,
		"It is %d o'clock %s, Day of %s, %d%s the Month of %s.\n\rThe Myth started up at %s\rThe system time is %s\r",
...
void do_who( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( arg, "bounty" ) )
	{
		status_who( ch );
		return;
	}

	if ( !str_cmp( arg, "ftalk" ) && (IS_MORE(ch, MORE_FTALK) || IS_IMMORTAL(ch)))
	{
		ftalk_who( ch );
		return;
	}
	...
	*/
	...
	for ( d = descriptor_list; d != NULL; d = d->next )
	{
		...
		*/
		...
		if (!IS_IMMORTAL(d->character) ) nTotal++;
		...
		*/
		...
		default:
			break;
		case MAX_LEVEL -  0:
			if (!str_cmp(wch->name,"Morglum")) class = "#CTwi#rs#Cted#n      ";
		...
		case MAX_LEVEL -  9:
			...
			if ( IS_DEMON(wch) )
			{
				...
				else if (wch->race < 5 ) class =  "Lost Soul";
				else if (wch->race < 10) class =  "LesserShadow";
				else if (wch->race < 15) class =  "Shadow";
				else if (wch->race < 20) class =  "GreaterShadow";
				else if (wch->race < 25) class =  "Shadow Lord";
				else if (wch->race < 30) class =  "Lesser Devil";
				else if (wch->race < 35) class =  "Devil";
				else if (wch->race < 40) class =  "Greater Devil";
				else if (wch->race < 45) class =  "Demon Knight";
				else if (wch->race < 49) class =  "Hell's Knight";
				else class =  "Lord of Hell";
			}
			else if ( IS_VAMPIRE(wch) )
			{
				if (wch->race <= 0) class =  "Avatar";
				else if (wch->race < 5 ) class =  "Recognised";
				else if (wch->race < 10) class =  "Known";
				else if (wch->race < 15) class =  "Revered";
				else if (wch->race < 20) class =  "Respected";
				else if (wch->race < 25) class =  "Clan Head";
				else if (wch->race < 30) class =  "Primogen";
				else if (wch->race < 35) class =  "Shiriff";
				else if (wch->race < 40) class =  "Herald";
				else if (wch->race < 45) class =  "Prince";
				else if (wch->race < 49) class =  "Archon";
				else class =  "Justicar";
			}
			else if ( IS_WEREWOLF(wch) )
			{
				if (wch->race <= 0) class =  "Avatar";
				else if (wch->race < 5 ) class =  "Garou";
				else if (wch->race < 10) class =  "Claith";
				else if (wch->race < 15) class =  "Fostern";
				else if (wch->race < 20) class =  "Adren";
				else if (wch->race < 25) class =  "Ahroun";
				else if (wch->race < 30) class =  "Athro";
				else if (wch->race < 35) class =  "Adult";
				else if (wch->race < 40) class =  "Elder";
				else if (wch->race < 45) class =  "Sept Leader";
				else if (wch->race < 49) class =  "Claw of Gaia";
				else class =  "Gaia's Rage";
			}
			else if ( IS_MAGE(wch) )
			{
				if (wch->race <= 0) class =  "Avatar";
				else if (wch->race < 5 ) class =  "Chanter";
				else if (wch->race < 10) class =  "Conjurer";
				else if (wch->race < 15) class =  "Magician";
				else if (wch->race < 20) class =  "Magus";
				else if (wch->race < 25) class =  "Sorcerer";
				else if (wch->race < 30) class =  "Wizard";
				else if (wch->race < 35) class =  "Magical Knight";
				else if (wch->race < 40) class =  "War Mage";
				else if (wch->race < 45) class =  "Battle Mage";
				else if (wch->race < 49) class =  "Mage Lord";
				else class =  "Chosen";
			}
			else
			{
				if (wch->race <= 0) class =  "Avatar";
				else if (wch->race < 5 ) class =  "Immortal";
				else if (wch->race < 10) class =  "Immortal";
				else if (wch->race < 15) class =  "Godling";
				else if (wch->race < 20) class =  "Godling";
				else if (wch->race < 25) class =  "Demigod";
				else if (wch->race < 30) class =  "Demigod";
				else if (wch->race < 35) class =  "Lesser God";
				else if (wch->race < 40) class =  "Lesser God";
				else if (wch->race < 45) class =  "Greater God";
				else if (wch->race < 49) class =  "Greater God";
		...
		/*
		if(!str_cmp(wch->name,"Morglum") && wch->level >= 7)

		switch(number_range(1,4))
		{
		case 1: class = "#CTwi#rs#Cted#n      ";break;
		case 2: class = "#wUnknown#n      ";break;
		case 3: class = "#CThe Owner#n    ";break;
		case 4: class = "#wThe #bC#co#bd#ce#br#n    ";break;
		default: class = "#yImPLeMenToR#n  ";break;
		}
		*/
		...
		*/
		{
			...
			else if (IS_LICH(wch) )
			{
				if ( wch->vampgen <= 3 )
				{
					if ( wch->sex == SEX_FEMALE )
						sprintf( kav, ". %sLich Queen%s", openb, closeb );
					else
					    sprintf( kav, ". %sLich King%s", openb, closeb );
				}
				else
			...
			{
				if ( wch->vampgen <= 3 )
					sprintf( kav, ". %sBaal's Avatar%s", openb, closeb );
				else
			...
			{
				if ( wch->vampgen <= 3 )
					sprintf( kav, ". %sBlack Spiral Dancer Chief%s", openb, closeb );
				else
			...
			else if (IS_NEPHANDI(wch) )
			{
				if ( wch->vampgen <= 3 )
				    ...
				    sprintf( kav, ". %sNephandi%s", openb, closeb );
			}
			else if ( IS_DEMON(wch) )
			{
				if ( IS_SET(wch->act, PLR_DEMON) )
					sprintf(kav, ". %sDemon %s%s", openb,
						wch->sex == SEX_FEMALE ? "Mistress" : "Lord", closeb );
				else if ( IS_EXTRA(wch, EXTRA_PRINCE) )
					sprintf( kav, ". %sDemon Prince%s of %s%s", openb,
						wch->sex == SEX_FEMALE ? "ss" : "", wch->lord, closeb );
				else if ( IS_MORE( wch, MORE_OUTCAST ) )
					sprintf( kav, ". %sDemonic Rogue%s", openb, closeb);
			...
			else if ( IS_VAMPIRE(wch) )
			{
				...
				if (use_age)
				{
					...
					if ( IS_MORE( wch, MORE_OUTCAST ) )
						sprintf( kav, ". %sAnarch%s", openb, closeb );
			...
			else if ( IS_WEREWOLF(wch) )
			{
				if ( strlen(wch->clan) > 1 )
				{
					if (wch->vampgen == 2)
						...
						sprintf( kav, ". %s%s Scout%s", openb, wch->clan, closeb  );
					else
		...
		if (IS_IMMORTAL(wch) )
			...
			sprintf( immbuf + strlen(immbuf), "%-13s #b|#n %s%s%s\n\r",
				class,
				wch->name,
				wch->pcdata->title,
				kav );
		else
			sprintf( buf + strlen(buf), "%-13s #b|#n %s%s%s\n\r",
				class,
				wch->name,
				wch->pcdata->title,
				kav );
	...
	if (nMatch == 1)
		sprintf( buf2, "There is one visible player connected!\n\r");
	else
	    sprintf( buf2, "There are a total of %d visible players connected.\n\r", nMatch);

	if (immMatch == 1)
		sprintf( god, "There is one visible immortal connected!\n\r");
	else
	    sprintf( god, "There are a total of %d visible immortals connected.\n\r",immMatch);

	if (immMatch > 0 )
	{
		send_to_char("#b-----------------------------------#wIMMORTALS#b------------------------------------\n\r", ch);
		send_to_char(immbuf, ch);
	}
	if (nMatch > 0 )
	{
		send_to_char("#b------------------------------------#wPLAYERS#b-------------------------------------\n\r", ch);
		send_to_char( buf, ch );
	}
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg[0] == '\0' )
	{
		...
		if (IS_HIGHJUDGE(ch))
		{
			send_to_char( !IS_SET( ch->deaf, CHANNEL_CTALK )
				    ? " +CTALK"
				    : " -ctalk",
				ch );
		}
	...
	else
	{
		...
		else if ( !str_cmp( arg+1, "buildtalk") ) bit = CHANNEL_BUILD;
		...
		else if ((IS_SET(ch->act, PLR_CHAMPION) || IS_SET(ch->act, PLR_DEMON)
			    || IS_IMMORTAL(ch)) && !str_cmp( arg+1, "pray") ) bit = CHANNEL_PRAY;
		else if ( !str_cmp( arg+1, "ctalk" ) && IS_HIGHJUDGE(ch))bit = CHANNEL_CTALK;
...
void do_diagnose( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_VAMPIRE(victim)) victim->loc_hp[6] = 0;
...
DO_COM( do_peek )
{
	...
	if ( number_percent( ) <= ch->pcdata->learned[gsn_peek] &&
		    // Morglum : Stop mortals peeking at imm's
	!IS_IMMORTAL( victim ) )
	{
...
void do_affects( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "You are currently affected by: \n\r", ch);
	for( af = ch->affected; af; af = af->next )
	{
		sprintf( buf, "%s: %s %d for %d hour%s with bits %s.\n\r",
	...
	{
		send_to_char( "#y* You are surrounded by a crackling shield of lightning#n.\n\r", ch);
	}
	if ( IS_ITEMAFF(ch, ITEMA_FIRESHIELD) )
	{
		send_to_char( "#y* You are surrounded by a burning shield of fire#n.\n\r", ch);
	}
	if ( IS_ITEMAFF(ch, ITEMA_ICESHIELD) )
	{
		send_to_char( "#y* You are surrounded by a shimmering shield of ice#n.\n\r", ch);
	}
	if ( IS_ITEMAFF(ch, ITEMA_ACIDSHIELD) )
	{
		send_to_char( "#y* You are surrounded by a bubbling shield of acid#n.\n\r", ch);
	}
	if ( IS_ITEMAFF(ch, ITEMA_CHAOSSHIELD) )
	{
		send_to_char( "#y* You are surrounded by a swirling shield of chaos#n.\n\r", ch);
	}
	if ( IS_ITEMAFF(ch, ITEMA_REFLECT) )
	{
		send_to_char( "#y* You are surrounded by a flickering shield of darkness#n.\n\r", ch);
	}
	if ( IS_ITEMAFF(ch, ITEMA_SUNSHIELD) )
	{
		send_to_char( "#y* You are surrounded by a glowing halo of sunlight#n.\n\r", ch);
	}
	...
	if ( !IS_ITEMAFF(ch, ITEMA_SHOCKSHIELD) && !IS_ITEMAFF(ch, ITEMA_FIRESHIELD) &&
		    ...
		    &&  !IS_ITEMAFF(ch, ITEMA_SUNSHIELD) )
	{
		send_to_char( "#y* You are surrounded by no shields#n.\n\r", ch);
	}
