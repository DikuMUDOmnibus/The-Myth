...
void do_cast( CHAR_DATA *ch, char *argument )
{
 ...
 /* Morglum : to stop peeps casting skills :) */
    ...
    ||	 skill_lookup( arg1 ) == skill_lookup( "elbow"		)
    ||	 skill_lookup( arg1 ) == skill_lookup( "headbutt"	)
    ||	 skill_lookup( arg1 ) == skill_lookup( "sweep"		)
    ||	 skill_lookup( arg1 ) == skill_lookup( "knee"		)
    ||	 skill_lookup( arg1 ) == skill_lookup( "forked tongue"	)
    ||	 skill_lookup( arg1 ) == skill_lookup( "lashing tail"	)
    ||	 skill_lookup( arg1 ) == skill_lookup( "quill-like spines" )
    ||	 skill_lookup( arg1 ) == skill_lookup( "fastdraw"	)
    ||	 skill_lookup( arg1 ) == skill_lookup( "track"		)
    ||	 skill_lookup( arg1 ) == skill_lookup( "first aid"	)
    ||	 skill_lookup( arg1 ) == skill_lookup( "repair"		)
    ||	 skill_lookup( arg1 ) == skill_lookup( "spit of blood"	) )
    {
	send_to_char( "That is a skill, not a spell.\n\r", ch );
	return;
    }
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (obj->points > 0 && obj->item_type != ITEM_QUEST
	 && obj->item_type != ITEM_PAGE && obj->item_type != ITEM_ETOKEN )
    {
    ...
    case ITEM_ETOKEN:
	sprintf( buf, "Energy point value is %d.\n\r", obj->value[0] );
	send_to_char( buf, ch );
	break;
...
void spell_poison( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    &&	 get_tribe( victim, TRIBE_FIANNA ) > 2 ) return;
    if (!IS_NPC(victim) && victim->pcdata->atm >= number_percent())
	return;
...
/* Shim-mod 17/04/98 : New soulblade spell (random effects) */
...
*/
    ...
    switch ( weapontype )
    {
	default : strcpy(wpnname,"blade");	break;
	case 2  :
	case 11 : strcpy(wpnname,"dagger");	break;
	case 4  : strcpy(wpnname,"whip");	break;
	case 5  : strcpy(wpnname,"claw");	break;
	case 6  : strcpy(wpnname,"blaster");	break;
	case 7  :
	case 8  : strcpy(wpnname,"club");	break;
	case 9  : strcpy(wpnname,"grepper");	break;
	case 12 : strcpy(wpnname,"vacuum");	break;
    }
...
void spell_quest(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    obj_to_char(obj, ch);
    quest_object(ch,obj);
    if (ch->practice >= max) {ch->practice -= max; obj->level = max;}
...
void spell_brew( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
	send_to_char( "Oh dear...big bug...please inform Morglum.\n\r", ch );
	return;
    }
    obj->value[1] = sn;
...
*/
	...
	if (victim == NULL)
	{send_to_char("Error - please inform Morglum.\n\r",ch); return;}
    }
    else if ( !str_cmp( target_name, "raven" ) )
    {
...
void spell_scry( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( "You create a rip in reality.", ch, NULL, NULL, TO_CHAR );
    act( "$n creates a rip in reality.", ch, NULL, NULL, TO_ROOM );

    act( "A rip in reality appears from nowhere.", victim, NULL, NULL, TO_CHAR );
    act( "A rip in reality appears from nowhere.", victim, NULL, NULL, TO_ROOM );
    ...
    act( "You look through a rip in reality.", ch, NULL, NULL, TO_CHAR );
    act( "$n looks through a rip in reality.", ch, NULL, NULL, TO_ROOM );
    ...
    act( "A rip in reality closes.", victim, NULL, NULL, TO_CHAR );
    act( "A rip in reality closes.", victim, NULL, NULL, TO_ROOM );

    act( "You close the rip in reality.", ch, NULL, NULL, TO_CHAR );
    act( "$n closes the rip in reality.", ch, NULL, NULL, TO_ROOM );
