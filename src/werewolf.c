...
void do_porcupine( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You require the level 5 Metis power to use the Gift of the Porcupine.\n\r",ch);
	return;
    }
...
void do_obedience( CHAR_DATA *ch, char *argument )
{
    ...
    {
	STC( "You can't use obedience in safe rooms.\n\r", ch );
	return;
    }
...
void do_clap( CHAR_DATA *ch, char *argument )
{
	...
	for ( victim = ch->in_room->people; victim; victim = victim_next )
	{
	    ...
	    {
		guardian_message( victim );
		act4("$n just used Clap of Thunder on you.",ch,NULL,victim,TO_VICT);
	    }
	    victim->hit -= damage;
		...
		else if (IS_NPC(victim)) multi_hit(victim,ch,TYPE_UNDEFINED);
	    }
	}
	WAIT_STATE( ch, 24 );
	return;
...
void do_reshape( CHAR_DATA *ch, char *argument )
{
	...
	sprintf( buf3, "%s", strip_small_args( buf2 ) );
	FILL_STRING( obj->name, buf3 );
	FILL_STRING( obj->short_descr, argument );
...
void do_bodywrack( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( victim ) )
    {
	if ( IS_DEMON( victim ) && IS_DEMPOWER( victim, DEM_TOUGH ) )
	    return;
	if ( IS_WEREWOLF( victim ) && get_tribe( victim, TRIBE_GLASS_WALKERS ) > 4 )
	    return;
	if ( get_disc( victim, DISC_FORTITUDE ) > 4 )
	    return;
    }
...
void do_wwpowers( CHAR_DATA *ch, char *argument )
{
	...
	if (get_breed(ch, BREED_HOMID) > 4) strcat(buf," Mask");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
    ...
    {
	strcpy(buf,"Ragabash:");
	if (get_auspice(ch, AUSPICE_RAGABASH) > 1) strcat(buf," Blur");
	if (get_auspice(ch, AUSPICE_RAGABASH) > 2) strcat(buf," Silence");
	if (get_auspice(ch, AUSPICE_RAGABASH) > 3) strcat(buf," Tail");
	if (get_auspice(ch, AUSPICE_RAGABASH) > 4) strcat(buf," Moonbridge");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_auspice(ch, AUSPICE_THEURGE) > 2) strcat(buf," Control");
	if (get_auspice(ch, AUSPICE_THEURGE) > 3) strcat(buf," Spiritguardian");
	if (get_auspice(ch, AUSPICE_THEURGE) > 4) strcat(buf," Readaura");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_auspice(ch, AUSPICE_GALLIARD) > 3) strcat(buf," Klaive");
	if (get_auspice(ch, AUSPICE_GALLIARD) > 4) strcat(buf," Hearttwister");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_tribe(ch, TRIBE_BLACK_FURIES) > 3) strcat(buf," Hagswrinkles");
	if (get_tribe(ch, TRIBE_BLACK_FURIES) > 4) strcat(buf," Bodywrack");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
    if (get_tribe(ch, TRIBE_BONE_GNAWERS) > 0)
    {
	strcpy(buf,"Bone Gnawers: Pigeon");
	if (get_tribe(ch, TRIBE_BONE_GNAWERS) >= 1) strcat(buf, " Brew");
	if (get_tribe(ch, TRIBE_BONE_GNAWERS) > 3) strcat(buf, " Minorcreate");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_tribe(ch, TRIBE_CHILDREN_OF_GAIA) > 2) strcat(buf," Dazzle");
    ...
    {
	strcpy(buf,"Fianna:");
	if (get_tribe(ch, TRIBE_FIANNA) > 2) strcat(buf," Formillusion");
	if (get_tribe(ch, TRIBE_FIANNA) > 3) strcat(buf," Cloneobject");
	if (get_tribe(ch, TRIBE_FIANNA) > 4) strcat(buf," Zulo");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
    ...
    {
	strcpy(buf,"Glass Walkers: Entrap");
	if (get_tribe(ch,TRIBE_GLASS_WALKERS) > 4) strcat(buf," Circleattack");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
/*
...
*/
	...
	if (get_tribe(ch, TRIBE_SILENT_STRIDERS) > 4) strcat(buf, " Gate");
	if (get_tribe(ch, TRIBE_SILENT_STRIDERS) <= 4) strcat(buf, " Hunt");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_tribe(ch, TRIBE_SILVER_FANGS) > 3) strcat(buf," Shield");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if ( get_tribe(ch,TRIBE_STARGAZERS) > 4) strcat(buf," Grab");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_tribe(ch, TRIBE_WENDIGOS) > 1) strcat( buf, " Earthquake");
	if (get_tribe(ch, TRIBE_WENDIGOS) > 2) strcat( buf, " Chill");
	if (get_tribe(ch, TRIBE_WENDIGOS) > 3) strcat( buf, " Engulf");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
...
void do_tail( CHAR_DATA *ch, char *argument )
{
    ...
    damage( ch, victim, dam, gsn_tail + TYPE_ADD_AGG );
    return;
}
