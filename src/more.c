...
void do_fleshcraft( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_MAGE(victim))
	{
	    send_to_char( "You cannot fleshcraft Mages.\n\r", ch );
	    return;
	}
	if (IS_WEREWOLF(victim))
	{
	    send_to_char( "You cannot fleshcraft Werewolfs.\n\r", ch );
	    return;
	}
...
void do_bonecraft( CHAR_DATA *ch, char *argument )
{
    ...
    if (is_in(argument,"|Morglum*Rox*Alucard*Ryana*"))
    {
	send_to_char( "Illegal name.\n\r", ch );
	return;
    }
...
DO_COM( do_zuloform )
{
    ...
    else if (IS_BAALI(ch) && IS_POLYAFF(ch, POLY_ZULO))
    ...
    if (IS_BAALI(ch) && IS_POLYAFF(ch, POLY_ZULO))
	  act("A hideous monstrosity shrinks and distorts into the form of $n.",ch,NULL,NULL,TO_ROOM);
    else if (IS_WEREWOLF(ch) && IS_SET(ch->act, PLR_WOLFMAN))
	  act("A gigantic furry werewolf's body shrinks into the form of $n.",ch,NULL,NULL,TO_ROOM);
    else if (IS_WEREWOLF(ch))
	  act("A giant's body shrinks into the form of $n.",ch,NULL,NULL,TO_ROOM);
    else
    ...
    {
    send_to_char("Your body grows and distorts into a hideous monstrosity.\n\r",ch);
    act("$n's body grows and distorts into a hideous monstrosity.",ch,NULL,NULL,TO_ROOM);
    }
    else if ( IS_WEREWOLF(ch) && IS_SET(ch->act, PLR_WOLFMAN))
    {
	send_to_char("Your body grows into a gigantic furry werewolf.\n\r", ch );
	act("$n's body grows into a gigantic furry werewolf.",ch,NULL,NULL,TO_ROOM);
    }
    else if ( IS_WEREWOLF( ch ) )
    {
	send_to_char("Your body grows into a giant.\n\r", ch );
	act("$n's body grows into a giant.",ch,NULL,NULL,TO_ROOM);
    }
    else
    {
    ...
    ch->morph = str_dup("A gigantic furry werewolf");
    else if (IS_WEREWOLF(ch))
    ch->morph = str_dup("A giant");
    else
...
void do_plasmaform( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You require level 4 Vicissitude to change into Plasma Form.\n\r",ch);
	return;
    }
...
void do_portal( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_DEMON( ch ) && get_disc( ch, DISC_DAIMOINON ) < 3 )
    {
	if ( !IS_SET( ch->pcdata->disc[O_POWERS], DEM_PORTAL ) )
	{
	    send_to_char( "You have not been granted the gift of portal.\n\r", ch );
	    return;
	}
    }

    else if (get_disc(ch,DISC_DAIMOINON) < 3)
    {
...
void do_soultwist( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_DEMON( ch ) && get_disc( ch, DISC_DAIMOINON ) < 5 )
    {
	if ( !IS_SET( ch->pcdata->disc[O_POWERS], DEM_TWIST ) )
	{
	    send_to_char( "You have not been granted the gift of soultwist.\n\r", ch );
	    return;
	}
    }

    else if (get_disc(ch,DISC_DAIMOINON) < 5)
    {
    ...
    if ( ch == victim )
    {
	send_to_char( "Your soul isn't yours to destroy!\n\r", ch );
	return;
    }
...
void do_curse( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_DEMON( ch ) && get_disc( ch, DISC_DAIMOINON ) < 4 )
    {
	if ( !IS_SET( ch->pcdata->disc[O_POWERS], DEM_CURSE ) )
	{
	    send_to_char( "You have not been granted the power of curse.\n\r", ch );
	    return;
	}
    }

    else if (get_disc(ch,DISC_DAIMOINON) < 4)
    {
...
void do_tongue( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch == victim )
    {
	act( "You touch your nose with your long serpentine tongue.",ch,NULL,victim,TO_CHAR);
	act( "$n touches $s nose with his long serpentine tongue.",ch,NULL,victim,TO_ROOM );
	return;
    }
...
void do_silencedeath( CHAR_DATA *ch, char *argument )
{
    ...
    &&	get_auspice(ch,AUSPICE_RAGABASH) < 4)
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
/* Morglum : Stop people using spit on themselves */
    if ( ch == victim )
    {
	send_to_char( "1 agg, 2 agg, 3 agg, DEAD!\n\r", ch );
	return;
    }
	...
	for (obj_lose = victim->carrying; obj_lose != NULL; obj_lose = obj_next)
	{
	    ...
	    if ( obj_lose->item_type == ITEM_BOOK ||
		 obj_lose->item_type == ITEM_PAGE )
		continue;
...
void do_hagswrinkles( CHAR_DATA *ch, char *argument )
{
    ...
    &&	 get_tribe(ch,TRIBE_BLACK_FURIES) < 4 )
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
// remade cots into cults - Morglum
...
*/
	...
	if (strlen(ch->pcdata->coterie) > 1 && strlen(ch->pcdata->leader) > 1 )
	{
	    ...
	    sprintf( buf, "The %s cult, lead by %s:\n\r",
	    ...
	    /* Display the following information about the coterie members:
             ...
             */
	    send_to_char("#b[      #wName#b     ] [  #wRank#b  ] [    #wClass#b    ] [  #wHits#b  ] [     #wExp#b     ] [#wStatus#b]\n\r", ch );
...
*/
		...
		if ( !str_cmp(ch->pcdata->coterie,gch->pcdata->coterie) )
		{
			...
			strcpy( arg, "Demon" );
		    else if ( gch->class == CLASS_BAALI )
			strcpy( arg, "Baali" );
		    else if ( gch->class == CLASS_BLACK_SPIRAL_DANCER )
			strcpy( arg, "B.S.D." );
		    else if ( gch->class == CLASS_NEPHANDI )
			strcpy( arg, "Nephandi" );
		    else if ( gch->class == CLASS_GHOUL )
			strcpy( arg, "Ghoul" );
		    else
			strcpy( arg, "None" );

		    sprintf( buf,
		    "#b[#n%-15s#b] [#n%8s#b] [#n%-13s#b] [#n%-8d#b] [#n%-13d#b] [  #n%-3d#b ]\n\r",
			capitalize( gch->name ),
			IS_MORE(gch, MORE_LEADER) ? " Leader " :
			IS_MORE(gch, MORE_CHAMPION) ? "Champion" : " Member ",
			arg,
			gch->hit, gch->exp, gch->race );
			send_to_char( buf, ch );
		}
	    }
	    send_to_char("#r--------------------------------------------------------------------------------\n\r",ch);
	    send_to_char( "For more information type 'cult help'.\n\r", ch );
	    send_to_char("#r--------------------------------------------------------------------------------\n\r",ch);
	    return;
	}
	 ...
	 */
	send_to_char("#r--------------------------------------------------------------------------------\n\r",ch);
	send_to_char("                              -= Cult Options =-\n\r",ch);
	send_to_char("#r--------------------------------------------------------------------------------\n\r",ch);
	send_to_char("CHAMPION : Make a member into a champion.\n\r",ch);
	send_to_char("FOUND    : Founds a new Cult.\n\r",ch);
	send_to_char("INDUCT   : Inducts a member into your cult.\n\r",ch);
	send_to_char("INFO     : Lists information on your cult.\n\r",ch);
	send_to_char("LEAVE    : Leaves your current Cult.\n\r",ch);
	send_to_char("OUTCAST  : Outcasts a member from your cult.\n\r",ch);
//	send_to_char("READY    : Coterie rules are completed.\n\r",ch);
	...
	{
	    send_to_char( "But you are not a member of any cult!\n\r", ch );
	    return;
	}
...
*/
		...
		{
		    act( "Info -> $n has disbanded the cult.", ch, NULL, gch, TO_VICT );
		    FILL_STRING( gch->pcdata->coterie, "" );
		    FILL_STRING( gch->pcdata->leader, "" );
		}
		else
		    act( "Info -> $n has abandoned the cult.", ch, NULL, gch, TO_VICT );
	    }
	}
	if (IS_MORE(ch, MORE_LEADER))
	    send_to_char("You disband the cult.\n\r",ch);
	else
	    send_to_char("You abandon your brthren.\n\r",ch);
	REMOVE_BIT(ch->more, MORE_CHAMPION);
	FILL_STRING( ch->pcdata->coterie, "" );
	FILL_STRING( ch->pcdata->leader, "" );
	...
	{
	    send_to_char( "But you are already the member of a cult!\n\r", ch );
	    return;
	}
	if (argument[0] == '\0')
	{
	    send_to_char( "What do you wish to name your cult?\n\r", ch );
	    return;
	}
	if (strlen(argument) < 5 || strlen(argument) > 20 )
	{
	    send_to_char( "Your cults name should be between 5 and 20 characters long.\n\r", ch );
	    return;
	}
	...
	send_to_char("You have founded a new cult!\n\r",ch);
	FILL_STRING( ch->pcdata->coterie, argument );
	FILL_STRING( ch->pcdata->leader, ch->name );
	...
	{
	    send_to_char( "Only a cult leader or champion can induct new members.\n\r", ch );
	    return;
	}
	if (!IS_MORE(ch, MORE_CHAMPION) && IS_MORE(ch, MORE_LEADER))
	{
	    send_to_char( "Your cult has not yet been completely set up.\n\r", ch );
	    return;
	}
	if (argument[0] == '\0')
	{
...
*/
	...
	{
	    send_to_char( "They are already a member of your cult.\n\r", ch );
	    return;
	}
	...
	if (strlen(victim->pcdata->coterie) > 1)
	{
	    send_to_char( "They belong to a rival cult.\n\r", ch );
	    return;
	}
	...
	act("You induct $N into your cult.",ch,NULL,victim,TO_CHAR);
	act("$n inducts you into $s cult.",ch,NULL,victim,TO_VICT);
...
*/
	 ...
	 */
	FILL_STRING( victim->pcdata->coterie, ch->pcdata->coterie );
	FILL_STRING( victim->pcdata->leader, ch->pcdata->leader );
	...
	if (!IS_MORE(ch, MORE_LEADER) && !IS_MORE(ch, MORE_CHAMPION))
	{
	    send_to_char( "Only a cult leader or champion can outcast members.\n\r", ch );
	    return;
	}
	if (!IS_MORE(ch, MORE_CHAMPION) && IS_MORE(ch, MORE_LEADER))
	{
...
*/
	...
	{
	    send_to_char( "They are not a member of your cult.\n\r", ch );
	    return;
	}
	...
	act("You outcast $N from the cult.",ch,NULL,victim,TO_CHAR);
	act("$n outcasts you from the cult.",ch,NULL,victim,TO_VICT);

	/* Clear the person's coterie values, as they are no longer a
	 * coterie member.
	 */
	FILL_STRING( victim->pcdata->coterie, "" );
	FILL_STRING( victim->pcdata->leader, "" );
	REMOVE_BIT(victim->more, MORE_CHAMPION);
...
*/
	    ...
	    if ( str_cmp(ch->pcdata->leader,gch->pcdata->leader) ) continue;
	    if ( !str_cmp(ch->pcdata->coterie,gch->pcdata->coterie) )
		act( "Info -> $N has been banished from the cult!", gch, NULL, victim, TO_CHAR );
	}
	return;
    }
	...
	if (!IS_MORE(ch, MORE_LEADER))
	{
	    send_to_char( "Only a cult leader can make someone a champion.\n\r", ch );
	    return;
	}
...
*/
	...
	{
	    act("You make $N a cult champion.",ch,NULL,victim,TO_CHAR);
	    act("$n makes you a cult champion.",ch,NULL,victim,TO_VICT);
	    sprintf(buf,"Info -> $N has been made a champion!");
	    SET_BIT(victim->more, MORE_CHAMPION);
	}
	else
	{
	    act("You remove $N's cult champion position.",ch,NULL,victim,TO_CHAR);
	    act("$n removes your cult champion position.",ch,NULL,victim,TO_VICT);
	    sprintf(buf,"Info -> $N is no longer a cult champion!");
	    REMOVE_BIT(victim->more, MORE_CHAMPION);
	}
...
bool	is_coterie		args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
...
void do_ctalk( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("But you don't belong to a cult!\n\r",ch);
	return;
    }

    if ( argument[0] == '\0' )
    {
	send_to_char("Cult-talk what?\n\r",ch);
	return;
    }
    if ( IS_MORE(ch, MORE_LEADER) )
	act4("#y-=#n$n#y=-#n '#w$t#n'.", ch, argument, NULL, TO_CHAR);
    else
	act4("#y-#n$n#y-#n '#w$t#n'.", ch, argument, NULL, TO_CHAR);
...
//	if ( !IS_VAMPIRE(victim) ) continue;
	...
	if ( !is_coterie( ch, victim ) && !IS_HIGHJUDGE( victim ) ) continue;
	if ( !is_coterie( ch, victim ) && !str_cmp( ch->pcdata->leader, "Morglum" ) )
	    continue;
	if ( IS_SET( victim->deaf, CHANNEL_CTALK ) && !is_coterie( ch, victim ) )
	    continue;
	if ( IS_MORE(ch, MORE_LEADER) && is_coterie( ch, victim ) )
	    act4("#y-=#n$n#y=-#n '#w$t#n'.", ch, argument, victim, TO_VICT);
	else if ( is_coterie( ch, victim ) )
	    act4("#y-#n$n#y-#n '#w$t#n'.", ch, argument, victim, TO_VICT);
	else
	{
	    if ( IS_MORE( ch, MORE_LEADER ) )
		sprintf( buf, "(%s) #y-=#n%s#y=-#n '#w$t#n'.", ch->pcdata->leader, ch->name );
	    else
		sprintf( buf, "(%s) #y-#n%s#y-#n '#w$t#n'.", ch->pcdata->leader, ch->name );
	    act4( buf, ch, argument, victim, TO_VICT );
	}
    }
    return;
}

bool is_coterie( CHAR_DATA *ch, CHAR_DATA *victim )
{
    if ( strlen( victim->pcdata->coterie ) < 2 ) return FALSE;
    if ( strlen( victim->pcdata->leader ) < 2 ) return FALSE;
    if ( str_cmp( ch->pcdata->coterie, victim->pcdata->coterie ) )
	return FALSE;
    if ( str_cmp( ch->pcdata->leader, victim->pcdata->leader ) )
	return FALSE;
...
void do_hand( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->in_room != NULL && ch->in_room->light > 0 ) --ch->in_room->light;
	return;
    }
...
void do_flamebolt( CHAR_DATA *ch, char *argument )
{
	...
	{
	    guardian_message( victim );
	    act4("$n just failed to use Engulf on you.",ch,NULL,victim,TO_VICT);
	}
	return;
    }
...
void do_engulf( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_DEMON( ch ) )
    {
	if ( get_tribe(ch,TRIBE_WENDIGOS) < 4 )
    {
	send_to_char("Huh?\n\r",ch);
	return;
    }
...
void do_jail( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_SET( ch->pcdata->disc[O_POWERS], DEM_JAIL ) )
    {
        send_to_char( "You have not been granted the power of Jail.\n\r", ch );
        return;
    }
