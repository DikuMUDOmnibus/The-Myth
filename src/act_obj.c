...
*/
	...
	if ( !IS_NPC(ch) && ch->pcdata->condition[COND_FULL] > 50 &&
	    ...
	    obj->item_type != ITEM_PILL && obj->item_type != ITEM_ETOKEN )
	{
    ...
    if ( !IS_IMMORTAL( ch ) )
    {
	if ( obj->item_type == ITEM_ETOKEN
	&&	( IS_NPC( ch ) || !IS_DEMON( ch ) ) )
	{
...
*/
	...
	{
	    ch->pcdata->power[POWER_CURRENT] += obj->value[0];
	    ch->pcdata->power[POWER_TOTAL] += obj->value[0];
	}
	break;
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    {
	strcpy( buf, "You drain 5000 energy points from $p." );
...
void do_recharge( CHAR_DATA *ch, char *argument )
{
 ...
 //   if (value < 1) value = 1; else if (value > 50) value = 50;
	...
	if (value > 100)
	 {
	if (IS_MAGE(ch)) send_to_char("You perform a mudra over the card, stabilising it's power.\n\r", ch);
	 else
	 {
		send_to_char("The unstable card explodes!\n\r", ch);
		extract_obj(obj);
	}
}
...
void quest_object( CHAR_DATA *ch, OBJ_DATA *obj )
{
    static const sh_int quest_selection[] =
    {
	 ...
	 5005, 5011, 5012,  5013, 2902, 1352, 2348, 2361, 3005, 5013,
	 300,  303,  307,   7216, 1100, 100,  30315,5110, 6001, 3050,
	 301,  5230, 30302, 663,  7303, 2915, 2275, 8600, 8601, 8602,
	 8603, 5030, 9321,  6010, 1304, 1307, 1332, 1333, 1342, 1356,
	 1361, 2304, 2322,  2331, 2382, 8005, 5300, 5302, 5309, 5310,
	 5311, 4000, 601,   664,  900,  906,  923,  311,  7203, 7206,
	 5214, 5223, 5228,  2804, 1612, 5207, 9302, 5301, 5224, 7801,
	 9313, 6304, 2003,  3425, 3423, 608,  1109, 30319,8903, 9317,
	 9307, 4050, 911,   2204, 4100, 3428, 310,  5113, 3402, 5319,
	 6512, 5114, 913,   30316,2106, 8007, 6601, 2333, 3610, 2015,
	 5022, 1394, 2202,  1401, 6005, 1614, 647,  1388, 9311, 3604,
	 4701, 30325,6106,  7190, 9322, 1384, 3412, 2342, 1374, 2210,
	 2332, 2901, 7200,  7824, 3410, 2013, 1510, 8306, 3414, 2005,
	 1101
    };

// 142 coz thats how many quest items are listed above - Morglum
    int object;
    if (obj == NULL || obj->item_type != ITEM_QUESTCARD) return;

    object = number_range(obj->level, obj->level + 50);
    if (object < 1 || object > 142) object = 0;
...
void do_complete( CHAR_DATA *ch, char *argument )
{
   ...
   if (arg1[0] == '\0') {
	...
	for (i = 0; i < 4; i++) if (qobj->value[i] == -1) count++;
	...
	for (i = 0; i < 4; i++) if (qobj->value[i] == -1) count2++;
    if (count == count2) {
    ...
    if (count2 == 4)
		...
		act("$p now requires 1 more object!", ch, qobj, NULL, TO_CHAR);
    else {
		sprintf(buf, "$p now requires %d more objects!", 4- count2);
		act(buf, ch, qobj, NULL, TO_CHAR);
	}
    return;
}
