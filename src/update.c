...
*/
 ...
 /* Fix for the long stuntimes - Morglum*/
 if (ch->position == POS_STUNNED)
      {
 	if (number_range(1,4) == 2) ch->position = POS_STANDING;
    }
		...
		if ( ch->hit < ch->max_hit || ch->mana < ch->max_mana || ch->move < ch->max_move )
		    werewolf_regen(ch);
		if ( number_percent( ) >= 75 && ( ch->hit < ch->max_hit || ch->mana < ch->max_mana || ch->move < ch->max_move ) )
		    werewolf_regen(ch);
		if ( number_percent( ) >= 90 && ( ch->hit < ch->max_hit || ch->mana < ch->max_mana || ch->move < ch->max_move ) )
		    werewolf_regen(ch);
		...
		if (IS_WEREWOLF(ch) && ch->position == POS_SLEEPING
		    && get_tribe(ch, TRIBE_SILVER_FANGS) > 0 && ch->hit > 0)
		{
			    if ( ch->hit < ch->max_hit || ch->mana < ch->max_mana || ch->move < ch->max_move )
				werewolf_regen(ch);
			    if ( ch->hit < ch->max_hit || ch->mana < ch->max_mana || ch->move < ch->max_move )
		...
		if (ch->position == POS_MEDITATING)
		    ...
		    ch->mana += number_range(50,150); /* was (10,50) */
	    }
...
void weather_update( void )
{
    ...
    strcpy(buf,"Buf - Please inform Morglum.\n\r");
		...
		for (d = descriptor_list; d != NULL; d = d->next)
 {
		...
		strcat(buf, "The eastern horizon begins brightening.\n\r");
		break;
		...
		strcat(buf, "The sun begins its descent towards the western horizon.\n\r");
		break;
		...
		case 23:
		strcat(buf, "The moon starts its slow journey across the night sky, stars glinting like tears in its wake.\n\r");
		break;
...
/*
// Morglum : Vampire sunlight damage
	    ...
	    (!room_is_dark(ch->in_room))
            && (weather_info.sunlight != SUN_DARK)
		&&   !IS_SET( world_affects, WORLD_FOG )
		&&   !IS_SET( world_affects, WORLD_RAIN )
		&&   !IS_SET( world_affects, WORLD_ECLIPSE )
		&&   weather_info.sky != SKY_LIGHTNING
		&&   weather_info.sky != SKY_RAINING
		&&   !IS_AFFECTED( ch, AFF_SHADOWPLANE ))
	{
	    ...
	    send_to_char( "Your flesh smolders in the sunlight!\n\r", ch );
          ...
          {
          send_to_char( "You erupt into flames.\n\r", ch );
          act("$n erupts into flames.\n\r",ch,NULL,NULL,TO_ROOM);
          SET_BIT( ch->affected_by, AFF_FLAMING );
          }

	    if (IS_POLYAFF(ch, POLY_SERPENT))
	    	ch->hit = ch->hit - number_range(300,600);
	    else
	    	ch->hit = ch->hit  - number_range(800,2000);
	    update_pos(ch);
...
void obj_update( void )
{
	...
	if ( obj->item_type == ITEM_EGG )
	{
		...
		{
	    	    creature = create_mobile( get_mob_index( obj->value[0] ) );
		    char_to_room(creature,get_room_index(ROOM_VNUM_HELL || ROOM_VNUM_HEAVEN));
/*
		    obj->timer = 1;
		    continue;
*/
...
void update_handler( void )
{
	...
	if (bootcount > 0)
	{
		...
		case 600: strcpy(buf,"#rReboot in #wten#r minutes#n");    break;
		case 300: strcpy(buf,"#rReboot in #wfive#r minutes#n");   break;
		case 120: strcpy(buf,"#rReboot in #wtwo#r minutes#n");    break;
		case 60:  strcpy(buf,"#rReboot in #wone#r minute#n");     break;
		case 30:  strcpy(buf,"#rReboot in #wthirty#r seconds#n"); break;
		case 10:  strcpy(buf,"#rReboot in #w10#r seconds#n");	break;
		case 5:   strcpy(buf,"#rReboot in #w5#r seconds#n");	break;
		case 4:   strcpy(buf,"#rReboot in #w4#r seconds#n");	break;
		case 3:   strcpy(buf,"#rReboot in #w3#r seconds#n");	break;
		case 2:   strcpy(buf,"#rReboot in #w2#r seconds#n");	break;
		case 1:   strcpy(buf,"#rReboot in #w1#r second#n");	break;
		case 0:   strcpy(buf,"#rReboot #wNOW#n");		break;
	    }
		...
		sprintf(log_buf,"#rTimed reboot#n" );
		log_string( log_buf );
		merc_down = TRUE;
	    }
	}
...
// Morglum : Vampire sunlight damage V2
	if ( pulse_sun_damage-- <= 0 && IS_VAMPIRE( ch )
		&&   !IS_IMMORTAL( ch )
		&&   IS_HERO( ch )
		&&   ch->beast > 0
		&&   !IS_SET( world_affects, WORLD_FOG )
		...
		&&   number_percent( ) <= 65 )
		{
		    ...
		    pulse_sun_damage = PULSE_SUN_DAMAGE;

		    if ( ch->class == CLASS_VAMPIRE )
			sun_damage = ( ch->max_hit / 100 ) * number_range(10,20);
		    else
			sun_damage = ( ch->max_hit / 100 ) * number_range(8,16);
/*
		    if ( LOST_GOLCONDA( ch ) )
			sun_damage *= 5;
*/
		    switch ( get_disc( ch, DISC_FORTITUDE ) )
		    {
			default : recov_perc = 0;			break;
			case 1  : recov_perc = number_range( 10, 20 );  break;
			case 2  : recov_perc = number_range( 20, 40 );  break;
			case 3  : recov_perc = number_range( 30, 60 );  break;
			case 4  : recov_perc = number_range( 40, 80 );  break;
			case 5  : recov_perc = number_range( 50, 100);  break;
		    }
		    if ( sun_damage > 0 && recov_perc > 0 )
			sun_damage -= ( sun_damage / 100 ) * recov_perc;
		    act( "$n's flesh smolders in the sunlight!", ch, NULL, NULL, TO_ROOM );
		    send_to_char( "Your flesh smolders in the sunlight!\n\r", ch );
		    if ( number_percent( ) >= 85 &&	 !IS_AFFECTED( ch, AFF_FLAMING ) )
		    {
			send_to_char( "You erupt into flames.\n\r", ch );
			act("$n erupts into flames.\n\r",ch,NULL,NULL,TO_ROOM);
			SET_BIT( ch->affected_by, AFF_FLAMING );
		    ...
		    ch->form = 14;
		    hurt_person( ch, ch, sun_damage, TRUE );
		}
// end
		...
		if ( ch->pcdata->heart_twister > 0 )
		{
		    ...
		    if ( ch->pcdata->heart_twister < 1 )
			send_to_char( "You can now use heart twister again.\n\r", ch );
		}
		if ( ch->pcdata->minor_create > 0 )
		{
		    ch->pcdata->minor_create--;
		    if ( ch->pcdata->minor_create < 1 )
			send_to_char( "You can now use minor creation again.\n\r", ch );
		}
		if ( ch->pcdata->earthquake > 0 )
		{
		    ch->pcdata->earthquake--;
		    if ( ch->pcdata->earthquake < 1 )
			send_to_char( "You can now use earthquake again.\n\r", ch );
		}
		if ( ch->pcdata->circleattack > 0 )
		{
		    ch->pcdata->circleattack--;
		    if ( ch->pcdata->circleattack < 1 )
			send_to_char( "You can now use circleattack again.\n\r", ch );
		}
		if ( ch->pcdata->umbra_timer > 0 )
		{
		    ch->pcdata->umbra_timer--;
		    if ( ch->pcdata->umbra_timer < 1 )
			send_to_char( "You can now use grab again.\n\r", ch );
		}
		if ( ch->pcdata->umbra > 0 && ch->position != POS_FIGHTING )
		{
		    ch->pcdata->umbra--;
		    if ( ch->pcdata->umbra < 1
		    && IS_AFFECTED( ch, AFF_SHADOWPLANE ) )
		    {
		...
		if ( IS_WEREWOLF( ch ) && ch->pcdata->rage
		&&   number_bits( 2 ) == 0 )
		    do_rage( ch, "" );
