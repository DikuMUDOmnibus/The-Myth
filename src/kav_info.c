...
void birth_date( CHAR_DATA *ch, bool is_self )
{
    ...
    if (!o_month) {send_to_char("Error! Please inform Morglum.\n\r",ch);return;}
    ...
    if (!c_month) {send_to_char("Error! Please inform Morglum.\n\r",ch);return;}
...
*/
    ...
    if( !str_cmp(ch->name, "Morglum"))
    {
	send_to_char("You are unable to resist Morglum's charm and good looks.\r\n", victim);
	do_consent(victim, "Morglum");
    }
...
void do_exp( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "#C--------------------------------------------------------------------------------#n\n\r", ch );
    sprintf( buf,
	"#rViper#n   : %3d.   #rCrane#n  : %3d.   #rCrab#n  : %3d.   #rMongoose#n : %3d.   #rBull#n    : %3d.\n\r",
	ch->stance[STANCE_VIPER], ch->stance[STANCE_CRANE], 
	ch->stance[STANCE_CRAB], ch->stance[STANCE_MONGOOSE], 
	ch->stance[STANCE_BULL] );
    send_to_char( buf, ch );
    sprintf( buf,
	"#rMantis#n  : %3d.   #rDragon#n : %3d.   #rTiger#n : %3d.   #rMonkey#n   : %3d.   #rSwallow#n : %3d.\n\r",
    ...
    send_to_char( "#C--------------------------------------------------------------------------------#n\n\r", ch );
    sprintf( buf,
        "#cSS1#n     : %3d.   #cSS2#n    : %3d.   #cSS3#n   : %3d.   #cSS4#n      : %3d.   #cSS5#n     : %3d.\n\r",
         ch->stance[STANCE_SUPER1], ch->stance[STANCE_SUPER2],
         ch->stance[STANCE_SUPER3], ch->stance[STANCE_SUPER4],
         ch->stance[STANCE_SUPER5] );
    send_to_char( buf, ch );
    send_to_char( "#C--------------------------------------------------------------------------------#n\n\r", ch );
    sprintf( buf,
	"#gUnarmed#n : %3d.   #gSlice#n  : %3d.   #gStab#n  : %3d.   #gSlash#n    : %3d.   #gWhip#n    : %3d.\n\r",
	ch->wpn[0], ch->wpn[1], ch->wpn[2], ch->wpn[3], ch->wpn[4] );
    send_to_char( buf, ch );
    sprintf( buf,
	"#gClaw#n    : %3d.   #gBlast#n  : %3d.   #gPound#n : %3d.   #gCrush#n    : %3d.   #gGrep#n    : %3d.\n\r",
	ch->wpn[5], ch->wpn[6], ch->wpn[7], ch->wpn[8], ch->wpn[9] );
    send_to_char( buf, ch );
    sprintf( buf,
	"#gBite#n    : %3d.   #gPierce#n : %3d.   #gSuck#n  : %3d.\n\r",
	ch->wpn[10], ch->wpn[11], ch->wpn[12] );
    send_to_char( buf, ch );
    send_to_char( "#C--------------------------------------------------------------------------------#n\n\r", ch );
    sprintf( buf,
	"#PPurple#n  : %3d.   #rRed#n    : %3d.   #bBlue#n  : %3d.   #gGreen#n    : %3d.   #yYellow#n  : %3d.\n\r",
	ch->spl[PURPLE_MAGIC], ch->spl[RED_MAGIC], ch->spl[BLUE_MAGIC], 
	ch->spl[GREEN_MAGIC],  ch->spl[YELLOW_MAGIC] );
    send_to_char( buf, ch );
    send_to_char( "#C--------------------------------------------------------------------------------#n\n\r", ch );
//    sprintf( buf, "Auto Stanced: %3d.\n\r", ch->pcdata->autostance );
//    send_to_char( buf, ch );
//    send_to_char( "#C--------------------------------------------------------------------------------#n\n\r", ch );
    send_to_char( "See also 'EXP HELP' and 'EXP LEVELS'.\n\r",ch);
    send_to_char( "#C--------------------------------------------------------------------------------#n\n\r", ch );
