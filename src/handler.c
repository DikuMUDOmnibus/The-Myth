...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    if ((IS_NPC(ch) || !IS_MAGE(ch)) && IS_SET(obj->spectype, SITEM_MAGE))
    {
	act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
...
/*
    if ((IS_NPC(ch) || !IS_BORG(ch)) && IS_SET(obj->spectype, SITEM_BORG))
    {
	act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
...
*/
    ...
    if ( IS_NPC(ch)) ch->armor -= apply_ac( obj, iWear );
    obj->wear_loc = iWear;
...
/*
	if (IS_SET(obj->spectype, SITEM_BORG) && !IS_NPC(ch) && obj->item_type == ITEM_WEAPON)
	{
	    if (obj->value[3] == ch->pcdata->disc[0])
		SET_BIT(ch->itemaffect, ITEMA_BORG);
	}
*/	
...
/*
	...
	{
	    if (obj->value[3] == ch->pcdata->disc[0])
		REMOVE_BIT(ch->itemaffect, ITEMA_BORG);
	}
*/
	if (IS_SET(obj->quest, QUEST_ARTIFACT)) 
	    REMOVE_BIT(ch->itemaffect, ITEMA_ARTIFACT);
	if ( oldpos == WEAR_WIELD )
	{
...
bool is_abbr_name( const char *str, char *namelist )
{
    char name[MAX_INPUT_LENGTH];
...
// Finds a char in the area. - Morglum
CHAR_DATA *get_char_area( CHAR_DATA *ch, char *argument )
{
	...
	else if ( !can_see( ch, wch ) || ( !is_abbr_name( arg, wch->name ) &&
	    ( IS_NPC(wch) || !is_abbr_name( arg, wch->morph ))))
	    continue;
	else if ( wch->in_room == NULL ) continue;
	else if ( wch->in_room->area == NULL ) continue;
	else if ( wch->in_room->area != ch->in_room->area ) continue;
...
// Morglum : Mages with treusight spell can see vanished vamps 
    if ( !IS_NPC( ch ) && IS_AFFECTED( ch, AFF_TRUE_SIGHT ) )
    {
	if ( IS_VAMPAFF( victim, VAM_OBFUSCATE ) )
	{
	    if ( ( ch->spl[BLUE_MAGIC] / 48 )
...
// Morglum : You actually need Shadowsight into see in shadowplane
...
char *affect_bit_name( int vector )
{
    ...
    if ( vector & AFF_SHADOWSIGHT   ) strcat( buf, " umbra_vison"   );
    ...
    if ( vector & AFF_TRUE_SIGHT    ) strcat( buf, " true_sight"    );
    if ( vector & AFF_AURA_NO_MAGIC ) strcat( buf, " aura_no_magic" );
