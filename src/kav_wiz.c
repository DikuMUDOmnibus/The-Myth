...
void do_quest( CHAR_DATA *ch, char *argument )
{
    ...
    char morg [MAX_STRING_LENGTH];
    char morg1 [MAX_STRING_LENGTH];
    char endchar[10];
    OBJ_DATA *obj = NULL;
...
if (obj->specpower < 1)
          {
	    sprintf(buf,"You are unable to infuse any further powers within this demonic item.\n\r");
	    send_to_char( buf, ch );
          return;
          }
	...
	send_to_char("Ok.\n\r",ch);
	FILL_STRING( obj->questmaker, ch->name );
	FILL_STRING( obj->chpoweron, "You transform into a fine mist and seep into the ground." );
	FILL_STRING( obj->victpoweron, "$n transforms into a fine mist and seeps into the ground." );
	FILL_STRING( obj->chpoweroff, "You seep up from the ground and reform your body." );
	FILL_STRING( obj->victpoweroff, "A fine mist seeps up from the ground and reforms into $n." );
	FILL_STRING( obj->chpoweruse, "You activate $p." );
	FILL_STRING( obj->victpoweruse, "$n activates $p." );
	return;
    }
	...
	strcpy( morg1, arg3 );
	sprintf( morg, "%s", strip_small_args( morg1 ) );
	FILL_STRING( obj->name, morg );
	FILL_STRING( obj->short_descr, arg3 );
...
void do_clist( CHAR_DATA *ch, char *argument )
{
	...
	{
	    sprintf(buf,"[%s] No cult.\n\r",wch->name);
	    send_to_char(buf,ch);
	}
	else
	{
	    sprintf(buf,"[%s] The %s cult, lead by %s.\n\r", wch->name, 
